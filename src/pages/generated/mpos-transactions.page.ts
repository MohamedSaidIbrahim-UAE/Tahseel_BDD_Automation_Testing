/**
 * MposTransactions Page Object Model
 * 
 * Module: MPOS Transactions
 * URL: https://staging.tahseel.gov.ae/ManagePortal/help-desk/mpos-inquiry
 * 
 * This POM class encapsulates all interactions with the MPOS Transactions module.
 * Extends BasePage to inherit common functionality and utilities.
 * 
 * @category Page Objects
 * @module MposTransactions
 */
 ...
 * MposTransactions
 * 
 * Provides page object methods for MPOS Transactions module.
 * 
 * Usage:
 * ```typescript
 * const page = new MposTransactions(playwrightPage);
 * await page.navigateToModule();
 * await page.verifyPageLoaded();
 * ```
 */
export class MposTransactions extends BaseListPage {
  /**
   * Module name for identification
   */
  readonly moduleName = 'MPOS Transactions';

  /**
   * Module URL from audit data
   */
  readonly moduleUrl = 'https://staging.tahseel.gov.ae/ManagePortal/help-desk/mpos-inquiry';

  readonly selectors: Record<string, string> = {
    mainContainer: '[data-module], main, [role="main"]',
    // Form fields
    search_criteriaField: '[aria-label="Search Criteria"], label:has-text("Search Criteria") ~ input',
    departmentField: '[aria-label="Department"], label:has-text("Department") ~ input',
  ...
  async navigateToModule(): Promise<void> {
    ...
    await this.navigateToUrl('https://staging.tahseel.gov.ae/ManagePortal/help-desk/mpos-inquiry');
  }
