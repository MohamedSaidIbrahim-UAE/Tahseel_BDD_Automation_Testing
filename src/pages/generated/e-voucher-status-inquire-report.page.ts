/**
 * EVoucherStatusInquireReport Page Object Model
 * 
 * Module: E-Voucher Status Inquire Report
 * URL: https://staging.tahseel.gov.ae/ManagePortal/report-show/9681bc75-3c2a-49be-95c4-295671629468
 * 
 * This POM class encapsulates all interactions with the E-Voucher Status Inquire Report module.
 * Extends BasePage to inherit common functionality and utilities.
 * 
 * @category Page Objects
 * @module EVoucherStatusInquireReport
 */
...
/**
 * EVoucherStatusInquireReport
 * 
 * Provides page object methods for E-Voucher Status Inquire Report module.
 * 
 * Usage:
 * ```typescript
 * const page = new EVoucherStatusInquireReport(playwrightPage);
 * await page.navigateToModule();
 * await page.verifyPageLoaded();
 * ```
 */
export class EVoucherStatusInquireReport extends BaseListPage {
  /**
   * Module name for identification
   */
  readonly moduleName = 'E-Voucher Status Inquire Report';

  /**
   * Module URL from audit data
   */
  readonly moduleUrl = 'https://staging.tahseel.gov.ae/ManagePortal/report-show/9681bc75-3c2a-49be-95c4-295671629468';

  readonly selectors: Record<string, string> = {
    mainContainer: '[data-module], main, [role="main"]',
    // Form fields
    entityField: '[aria-label="Entity"], label:has-text("Entity") ~ input',
    refrence_numberField: '[aria-label="Refrence Number"], label:has-text("Refrence Number") ~ input',
    ...
    voucher_numberField: '[aria-label="Voucher Number"], label:has-text("Voucher Number") ~ input',
    usage_statusField: '[aria-label="Usage Status"], label:has-text("Usage Status") ~ input',
    stop_statusField: '[aria-label="Stop Status"], label:has-text("Stop Status") ~ input',
  ...
  async navigateToModule(): Promise<void> {
    ...
    await this.navigateToUrl('https://staging.tahseel.gov.ae/ManagePortal/report-show/9681bc75-3c2a-49be-95c4-295671629468');
  }
