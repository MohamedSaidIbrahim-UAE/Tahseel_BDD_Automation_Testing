/**
 * InquireTransactions Page Object Model
 * 
 * Module: Inquire Transactions
 * URL: https://staging.tahseel.gov.ae/ManagePortal/pre-auth-transaction/inquire
 * 
 * This POM class encapsulates all interactions with the Inquire Transactions module.
 * Extends BasePage to inherit common functionality and utilities.
 * 
 * @category Page Objects
 * @module InquireTransactions
 */
 ...
 * InquireTransactions
 * 
 * Provides page object methods for Inquire Transactions module.
 * 
 * Usage:
 * ```typescript
 * const page = new InquireTransactions(playwrightPage);
 * await page.navigateToModule();
 * await page.verifyPageLoaded();
 * ```
 */
export class InquireTransactions extends BaseListPage {
  /**
   * Module name for identification
   */
  readonly moduleName = 'Inquire Transactions';

  /**
   * Module URL from audit data
   */
  readonly moduleUrl = 'https://staging.tahseel.gov.ae/ManagePortal/pre-auth-transaction/inquire';
  ...
  async navigateToModule(): Promise<void> {
    ...
    await this.navigateToUrl('https://staging.tahseel.gov.ae/ManagePortal/pre-auth-transaction/inquire');
  }
  ...
  async fillField(fieldName: string, value: string): Promise<void> {
    ...
    if (selector) {
      await this.fillFilterInput(selector, value);
    }
  }
  ...
  async verifyTableHasData(): Promise<void> {
    await this.verifyTableHasRecords();
  }
  ...
  async searchTable(term: string): Promise<void> {
    if (this.selectors.searchInput) {
      await this.fillGridSearchImpl(term);
    }
  }
  ...
  async exportData(format: string = 'Excel'): Promise<void> {
    await this.clickExportAndSelectFormat(format);
  }
