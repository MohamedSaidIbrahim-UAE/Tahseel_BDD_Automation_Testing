/**
 * Step Definitions for Sharjah islamic bank refund report in period Module
 *
 * This file contains all step definitions for the Sharjah islamic bank refund report in period feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module sharjah-islamic-bank-refund-report-in-period.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { SharjahIslamicBankRefundReportInPeriod } from '../pages/generated/sharjah-islamic-bank-refund-report-in-period.page';
 ...
 */
let page: Page;
let sharjahIslamicBankRefundReportInPeriod: SharjahIslamicBankRefundReportInPeriod;
...
function initializePageObject(): void {
  ...
  sharjahIslamicBankRefundReportInPeriod = new SharjahIslamicBankRefundReportInPeriod(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await sharjahIslamicBankRefundReportInPeriod.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await sharjahIslamicBankRefundReportInPeriod.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await sharjahIslamicBankRefundReportInPeriod.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await sharjahIslamicBankRefundReportInPeriod.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await sharjahIslamicBankRefundReportInPeriod.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await sharjahIslamicBankRefundReportInPeriod.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await sharjahIslamicBankRefundReportInPeriod.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    sharjahIslamicBankRefundReportInPeriod.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await sharjahIslamicBankRefundReportInPeriod.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await sharjahIslamicBankRefundReportInPeriod.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await sharjahIslamicBankRefundReportInPeriod.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    sharjahIslamicBankRefundReportInPeriod.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    sharjahIslamicBankRefundReportInPeriod.selectors.submitButton as string
  );
});
