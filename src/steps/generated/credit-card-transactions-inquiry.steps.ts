/**
 * Step Definitions for Credit Card Transactions Inquiry Module
 *
 * This file contains all step definitions for the Credit Card Transactions Inquiry feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module credit-card-transactions-inquiry.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { CreditCardTransactionsInquiry } from '../pages/generated/credit-card-transactions-inquiry.page';
 ...
 */
let page: Page;
let creditCardTransactionsInquiry: CreditCardTransactionsInquiry;
...
function initializePageObject(): void {
  ...
  creditCardTransactionsInquiry = new CreditCardTransactionsInquiry(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await creditCardTransactionsInquiry.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await creditCardTransactionsInquiry.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await creditCardTransactionsInquiry.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await creditCardTransactionsInquiry.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await creditCardTransactionsInquiry.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await creditCardTransactionsInquiry.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await creditCardTransactionsInquiry.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    creditCardTransactionsInquiry.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await creditCardTransactionsInquiry.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await creditCardTransactionsInquiry.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await creditCardTransactionsInquiry.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    creditCardTransactionsInquiry.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    creditCardTransactionsInquiry.selectors.submitButton as string
  );
});
