/**
 * Step Definitions for Inquire Topup Transaction Module
 *
 * This file contains all step definitions for the Inquire Topup Transaction feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module inquire-topup-transaction.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { InquireTopupTransaction } from '../pages/generated/inquire-topup-transaction.page';
 ...
 */
let page: Page;
let inquireTopupTransaction: InquireTopupTransaction;
...
function initializePageObject(): void {
  ...
  inquireTopupTransaction = new InquireTopupTransaction(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await inquireTopupTransaction.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await inquireTopupTransaction.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await inquireTopupTransaction.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await inquireTopupTransaction.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await inquireTopupTransaction.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await inquireTopupTransaction.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await inquireTopupTransaction.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    inquireTopupTransaction.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await inquireTopupTransaction.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await inquireTopupTransaction.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await inquireTopupTransaction.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    inquireTopupTransaction.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    inquireTopupTransaction.selectors.submitButton as string
  );
});
