/**
 * Step Definitions for Payment method summary report Module
 *
 * This file contains all step definitions for the Payment method summary report feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module payment-method-summary-report.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { PaymentMethodSummaryReport } from '../pages/generated/payment-method-summary-report.page';
 ...
 */
let page: Page;
let paymentMethodSummaryReport: PaymentMethodSummaryReport;
...
function initializePageObject(): void {
  ...
  paymentMethodSummaryReport = new PaymentMethodSummaryReport(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await paymentMethodSummaryReport.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await paymentMethodSummaryReport.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await paymentMethodSummaryReport.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await paymentMethodSummaryReport.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await paymentMethodSummaryReport.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await paymentMethodSummaryReport.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await paymentMethodSummaryReport.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    paymentMethodSummaryReport.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await paymentMethodSummaryReport.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await paymentMethodSummaryReport.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await paymentMethodSummaryReport.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    paymentMethodSummaryReport.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    paymentMethodSummaryReport.selectors.submitButton as string
  );
});
