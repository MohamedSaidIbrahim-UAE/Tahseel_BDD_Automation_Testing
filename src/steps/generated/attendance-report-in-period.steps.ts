/**
 * Step Definitions for attendance report in period Module
 *
 * This file contains all step definitions for the attendance report in period feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module attendance-report-in-period.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { AttendanceReportInPeriod } from '../pages/generated/attendance-report-in-period.page';
 ...
 */
let page: Page;
let attendanceReportInPeriod: AttendanceReportInPeriod;
...
function initializePageObject(): void {
  ...
  attendanceReportInPeriod = new AttendanceReportInPeriod(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await attendanceReportInPeriod.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await attendanceReportInPeriod.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await attendanceReportInPeriod.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await attendanceReportInPeriod.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await attendanceReportInPeriod.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await attendanceReportInPeriod.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await attendanceReportInPeriod.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    attendanceReportInPeriod.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await attendanceReportInPeriod.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await attendanceReportInPeriod.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await attendanceReportInPeriod.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    attendanceReportInPeriod.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    attendanceReportInPeriod.selectors.submitButton as string
  );
});
