/**
 * Step Definitions for UAE central bank settlement report in period Module
 *
 * This file contains all step definitions for the UAE central bank settlement report in period feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module uae-central-bank-settlement-report-in-period.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { UaeCentralBankSettlementReportInPeriod } from '../pages/generated/uae-central-bank-settlement-report-in-period.page';
 ...
 */
let page: Page;
let uaeCentralBankSettlementReportInPeriod: UaeCentralBankSettlementReportInPeriod;
...
function initializePageObject(): void {
  ...
  uaeCentralBankSettlementReportInPeriod = new UaeCentralBankSettlementReportInPeriod(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await uaeCentralBankSettlementReportInPeriod.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await uaeCentralBankSettlementReportInPeriod.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await uaeCentralBankSettlementReportInPeriod.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await uaeCentralBankSettlementReportInPeriod.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await uaeCentralBankSettlementReportInPeriod.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await uaeCentralBankSettlementReportInPeriod.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await uaeCentralBankSettlementReportInPeriod.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    uaeCentralBankSettlementReportInPeriod.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await uaeCentralBankSettlementReportInPeriod.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await uaeCentralBankSettlementReportInPeriod.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await uaeCentralBankSettlementReportInPeriod.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    uaeCentralBankSettlementReportInPeriod.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    uaeCentralBankSettlementReportInPeriod.selectors.submitButton as string
  );
});
