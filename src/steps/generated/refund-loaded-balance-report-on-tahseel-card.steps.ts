/**
 * Step Definitions for Refund loaded balance report on Tahseel card Module
 *
 * This file contains all step definitions for the Refund loaded balance report on Tahseel card feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module refund-loaded-balance-report-on-tahseel-card.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { RefundLoadedBalanceReportOnTahseelCard } from '../pages/generated/refund-loaded-balance-report-on-tahseel-card.page';
 ...
 */
let page: Page;
let refundLoadedBalanceReportOnTahseelCard: RefundLoadedBalanceReportOnTahseelCard;
...
function initializePageObject(): void {
  ...
  refundLoadedBalanceReportOnTahseelCard = new RefundLoadedBalanceReportOnTahseelCard(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await refundLoadedBalanceReportOnTahseelCard.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await refundLoadedBalanceReportOnTahseelCard.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await refundLoadedBalanceReportOnTahseelCard.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await refundLoadedBalanceReportOnTahseelCard.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await refundLoadedBalanceReportOnTahseelCard.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await refundLoadedBalanceReportOnTahseelCard.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await refundLoadedBalanceReportOnTahseelCard.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    refundLoadedBalanceReportOnTahseelCard.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await refundLoadedBalanceReportOnTahseelCard.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await refundLoadedBalanceReportOnTahseelCard.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await refundLoadedBalanceReportOnTahseelCard.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    refundLoadedBalanceReportOnTahseelCard.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    refundLoadedBalanceReportOnTahseelCard.selectors.submitButton as string
  );
});
