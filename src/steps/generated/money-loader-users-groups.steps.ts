/**
 * Step Definitions for Money Loader Users Groups Module
 *
 * This file contains all step definitions for the Money Loader Users Groups feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module money-loader-users-groups.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { MoneyLoaderUsersGroups } from '../pages/generated/money-loader-users-groups.page';
 ...
 */
let page: Page;
let moneyLoaderUsersGroups: MoneyLoaderUsersGroups;
...
function initializePageObject(): void {
  ...
  moneyLoaderUsersGroups = new MoneyLoaderUsersGroups(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await moneyLoaderUsersGroups.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await moneyLoaderUsersGroups.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await moneyLoaderUsersGroups.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await moneyLoaderUsersGroups.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await moneyLoaderUsersGroups.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await moneyLoaderUsersGroups.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await moneyLoaderUsersGroups.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    moneyLoaderUsersGroups.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await moneyLoaderUsersGroups.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await moneyLoaderUsersGroups.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await moneyLoaderUsersGroups.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    moneyLoaderUsersGroups.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    moneyLoaderUsersGroups.selectors.submitButton as string
  );
});
