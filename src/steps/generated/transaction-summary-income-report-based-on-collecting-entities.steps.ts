/**
 * Step Definitions for Transaction summary income report based on collecting entities Module
 *
 * This file contains all step definitions for the Transaction summary income report based on collecting entities feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module transaction-summary-income-report-based-on-collecting-entities.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { TransactionSummaryIncomeReportBasedOnCollectingEntities } from '../pages/generated/transaction-summary-income-report-based-on-collecting-entities.page';
 ...
 */
let page: Page;
let transactionSummaryIncomeReportBasedOnCollectingEntities: TransactionSummaryIncomeReportBasedOnCollectingEntities;
...
function initializePageObject(): void {
  ...
  transactionSummaryIncomeReportBasedOnCollectingEntities = new TransactionSummaryIncomeReportBasedOnCollectingEntities(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await transactionSummaryIncomeReportBasedOnCollectingEntities.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await transactionSummaryIncomeReportBasedOnCollectingEntities.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await transactionSummaryIncomeReportBasedOnCollectingEntities.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await transactionSummaryIncomeReportBasedOnCollectingEntities.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await transactionSummaryIncomeReportBasedOnCollectingEntities.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await transactionSummaryIncomeReportBasedOnCollectingEntities.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await transactionSummaryIncomeReportBasedOnCollectingEntities.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    transactionSummaryIncomeReportBasedOnCollectingEntities.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await transactionSummaryIncomeReportBasedOnCollectingEntities.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await transactionSummaryIncomeReportBasedOnCollectingEntities.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await transactionSummaryIncomeReportBasedOnCollectingEntities.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    transactionSummaryIncomeReportBasedOnCollectingEntities.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    transactionSummaryIncomeReportBasedOnCollectingEntities.selectors.submitButton as string
  );
});
