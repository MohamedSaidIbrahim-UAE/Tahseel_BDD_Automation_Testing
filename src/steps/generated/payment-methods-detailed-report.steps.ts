/**
 * Step Definitions for Payment methods detailed report Module
 *
 * This file contains all step definitions for the Payment methods detailed report feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module payment-methods-detailed-report.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { PaymentMethodsDetailedReport } from '../pages/generated/payment-methods-detailed-report.page';
 ...
 */
let page: Page;
let paymentMethodsDetailedReport: PaymentMethodsDetailedReport;
...
function initializePageObject(): void {
  ...
  paymentMethodsDetailedReport = new PaymentMethodsDetailedReport(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await paymentMethodsDetailedReport.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await paymentMethodsDetailedReport.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await paymentMethodsDetailedReport.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await paymentMethodsDetailedReport.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await paymentMethodsDetailedReport.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await paymentMethodsDetailedReport.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await paymentMethodsDetailedReport.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    paymentMethodsDetailedReport.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await paymentMethodsDetailedReport.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await paymentMethodsDetailedReport.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await paymentMethodsDetailedReport.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    paymentMethodsDetailedReport.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    paymentMethodsDetailedReport.selectors.submitButton as string
  );
});
