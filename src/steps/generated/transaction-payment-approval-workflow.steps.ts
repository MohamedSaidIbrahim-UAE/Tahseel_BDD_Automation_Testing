/**
 * Step Definitions for Transaction Payment Approval Workflow Module
 *
 * This file contains all step definitions for the Transaction Payment Approval Workflow feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module transaction-payment-approval-workflow.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { TransactionPaymentApprovalWorkflow } from '../pages/generated/transaction-payment-approval-workflow.page';
 ...
 */
let page: Page;
let transactionPaymentApprovalWorkflow: TransactionPaymentApprovalWorkflow;
...
function initializePageObject(): void {
  ...
  transactionPaymentApprovalWorkflow = new TransactionPaymentApprovalWorkflow(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await transactionPaymentApprovalWorkflow.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await transactionPaymentApprovalWorkflow.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await transactionPaymentApprovalWorkflow.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await transactionPaymentApprovalWorkflow.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await transactionPaymentApprovalWorkflow.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await transactionPaymentApprovalWorkflow.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await transactionPaymentApprovalWorkflow.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    transactionPaymentApprovalWorkflow.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await transactionPaymentApprovalWorkflow.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await transactionPaymentApprovalWorkflow.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await transactionPaymentApprovalWorkflow.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    transactionPaymentApprovalWorkflow.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    transactionPaymentApprovalWorkflow.selectors.submitButton as string
  );
});
