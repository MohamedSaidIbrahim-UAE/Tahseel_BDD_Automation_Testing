/**
 * Step Definitions for Issue Workflow Management Module
 *
 * This file contains all step definitions for the Issue Workflow Management feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module issue-workflow-management.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { IssueWorkflowManagement } from '../pages/generated/issue-workflow-management.page';
 ...
 */
let page: Page;
let issueWorkflowManagement: IssueWorkflowManagement;
...
function initializePageObject(): void {
  ...
  issueWorkflowManagement = new IssueWorkflowManagement(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await issueWorkflowManagement.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await issueWorkflowManagement.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await issueWorkflowManagement.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await issueWorkflowManagement.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await issueWorkflowManagement.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await issueWorkflowManagement.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await issueWorkflowManagement.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    issueWorkflowManagement.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await issueWorkflowManagement.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await issueWorkflowManagement.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await issueWorkflowManagement.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    issueWorkflowManagement.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    issueWorkflowManagement.selectors.submitButton as string
  );
});
