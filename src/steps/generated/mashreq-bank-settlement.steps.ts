/**
 * Step Definitions for Mashreq Bank Settlement Module
 *
 * This file contains all step definitions for the Mashreq Bank Settlement feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module mashreq-bank-settlement.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { MashreqBankSettlement } from '../pages/generated/mashreq-bank-settlement.page';
 ...
 */
let page: Page;
let mashreqBankSettlement: MashreqBankSettlement;
...
function initializePageObject(): void {
  ...
  mashreqBankSettlement = new MashreqBankSettlement(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await mashreqBankSettlement.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await mashreqBankSettlement.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await mashreqBankSettlement.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await mashreqBankSettlement.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await mashreqBankSettlement.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await mashreqBankSettlement.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await mashreqBankSettlement.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    mashreqBankSettlement.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await mashreqBankSettlement.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await mashreqBankSettlement.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await mashreqBankSettlement.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    mashreqBankSettlement.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    mashreqBankSettlement.selectors.submitButton as string
  );
});
