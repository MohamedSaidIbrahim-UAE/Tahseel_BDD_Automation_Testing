/**
 * Step Definitions for View Deduct Pending Requests Module
 *
 * This file contains all step definitions for the View Deduct Pending Requests feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module view-deduct-pending-requests.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { ViewDeductPendingRequests } from '../pages/generated/view-deduct-pending-requests.page';
 ...
 */
let page: Page;
let viewDeductPendingRequests: ViewDeductPendingRequests;
...
function initializePageObject(): void {
  ...
  viewDeductPendingRequests = new ViewDeductPendingRequests(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await viewDeductPendingRequests.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await viewDeductPendingRequests.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await viewDeductPendingRequests.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await viewDeductPendingRequests.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await viewDeductPendingRequests.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await viewDeductPendingRequests.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await viewDeductPendingRequests.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    viewDeductPendingRequests.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await viewDeductPendingRequests.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await viewDeductPendingRequests.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await viewDeductPendingRequests.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    viewDeductPendingRequests.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    viewDeductPendingRequests.selectors.submitButton as string
  );
});
