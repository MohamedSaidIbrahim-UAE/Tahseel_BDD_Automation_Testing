/**
 * Step Definitions for Tahseel Account Deduct & Load Reqeust Module
 *
 * This file contains all step definitions for the Tahseel Account Deduct & Load Reqeust feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module tahseel-account-deduct-load-reqeust.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { TahseelAccountDeductLoadReqeust } from '../pages/generated/tahseel-account-deduct-load-reqeust.page';
 ...
 */
let page: Page;
let tahseelAccountDeductLoadReqeust: TahseelAccountDeductLoadReqeust;
...
function initializePageObject(): void {
  ...
  tahseelAccountDeductLoadReqeust = new TahseelAccountDeductLoadReqeust(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await tahseelAccountDeductLoadReqeust.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await tahseelAccountDeductLoadReqeust.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await tahseelAccountDeductLoadReqeust.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await tahseelAccountDeductLoadReqeust.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await tahseelAccountDeductLoadReqeust.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await tahseelAccountDeductLoadReqeust.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await tahseelAccountDeductLoadReqeust.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    tahseelAccountDeductLoadReqeust.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await tahseelAccountDeductLoadReqeust.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await tahseelAccountDeductLoadReqeust.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await tahseelAccountDeductLoadReqeust.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    tahseelAccountDeductLoadReqeust.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    tahseelAccountDeductLoadReqeust.selectors.submitButton as string
  );
});
