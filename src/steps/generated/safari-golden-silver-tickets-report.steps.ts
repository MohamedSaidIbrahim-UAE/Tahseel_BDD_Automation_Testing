/**
 * Step Definitions for Safari Golden & Silver Tickets Report Module
 *
 * This file contains all step definitions for the Safari Golden & Silver Tickets Report feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module safari-golden-silver-tickets-report.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { SafariGoldenSilverTicketsReport } from '../pages/generated/safari-golden-silver-tickets-report.page';
 ...
 */
let page: Page;
let safariGoldenSilverTicketsReport: SafariGoldenSilverTicketsReport;
...
function initializePageObject(): void {
  ...
  safariGoldenSilverTicketsReport = new SafariGoldenSilverTicketsReport(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await safariGoldenSilverTicketsReport.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await safariGoldenSilverTicketsReport.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await safariGoldenSilverTicketsReport.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await safariGoldenSilverTicketsReport.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await safariGoldenSilverTicketsReport.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await safariGoldenSilverTicketsReport.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await safariGoldenSilverTicketsReport.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    safariGoldenSilverTicketsReport.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await safariGoldenSilverTicketsReport.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await safariGoldenSilverTicketsReport.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await safariGoldenSilverTicketsReport.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    safariGoldenSilverTicketsReport.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    safariGoldenSilverTicketsReport.selectors.submitButton as string
  );
});
