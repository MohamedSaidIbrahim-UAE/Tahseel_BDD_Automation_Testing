/**
 * Step Definitions for Create Seller Refund Request Module
 *
 * This file contains all step definitions for the Create Seller Refund Request feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module create-seller-refund-request.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { CreateSellerRefundRequest } from '../pages/generated/create-seller-refund-request.page';
 ...
 */
let page: Page;
let createSellerRefundRequest: CreateSellerRefundRequest;
...
function initializePageObject(): void {
  ...
  createSellerRefundRequest = new CreateSellerRefundRequest(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await createSellerRefundRequest.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await createSellerRefundRequest.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await createSellerRefundRequest.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await createSellerRefundRequest.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await createSellerRefundRequest.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await createSellerRefundRequest.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await createSellerRefundRequest.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    createSellerRefundRequest.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await createSellerRefundRequest.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await createSellerRefundRequest.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await createSellerRefundRequest.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    createSellerRefundRequest.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    createSellerRefundRequest.selectors.submitButton as string
  );
});
