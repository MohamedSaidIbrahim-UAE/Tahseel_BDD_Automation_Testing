/**
 * Step Definitions for IBAN Registration Pending Requests Module
 *
 * This file contains all step definitions for the IBAN Registration Pending Requests feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module iban-registration-pending-requests.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { IbanRegistrationPendingRequests } from '../pages/generated/iban-registration-pending-requests.page';
 ...
 */
let page: Page;
let ibanRegistrationPendingRequests: IbanRegistrationPendingRequests;
...
function initializePageObject(): void {
  ...
  ibanRegistrationPendingRequests = new IbanRegistrationPendingRequests(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await ibanRegistrationPendingRequests.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await ibanRegistrationPendingRequests.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await ibanRegistrationPendingRequests.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await ibanRegistrationPendingRequests.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await ibanRegistrationPendingRequests.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await ibanRegistrationPendingRequests.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await ibanRegistrationPendingRequests.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    ibanRegistrationPendingRequests.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await ibanRegistrationPendingRequests.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await ibanRegistrationPendingRequests.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await ibanRegistrationPendingRequests.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    ibanRegistrationPendingRequests.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    ibanRegistrationPendingRequests.selectors.submitButton as string
  );
});
