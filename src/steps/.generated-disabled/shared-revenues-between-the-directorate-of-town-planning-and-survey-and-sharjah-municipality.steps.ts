/**
 * Step Definitions for Shared revenues between the Directorate Of Town Planning and Survey and Sharjah Municipality Module
 *
 * This file contains all step definitions for the Shared revenues between the Directorate Of Town Planning and Survey and Sharjah Municipality feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module shared-revenues-between-the-directorate-of-town-planning-and-survey-and-sharjah-municipality.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { SharedRevenuesBetweenTheDirectorateOfTownPlanningAndSurveyAndSharjahMunicipality } from '../pages/generated/shared-revenues-between-the-directorate-of-town-planning-and-survey-and-sharjah-municipality.page';
 ...
 */
let page: Page;
let sharedRevenuesBetweenTheDirectorateOfTownPlanningAndSurveyAndSharjahMunicipality: SharedRevenuesBetweenTheDirectorateOfTownPlanningAndSurveyAndSharjahMunicipality;
...
function initializePageObject(): void {
  ...
  sharedRevenuesBetweenTheDirectorateOfTownPlanningAndSurveyAndSharjahMunicipality = new SharedRevenuesBetweenTheDirectorateOfTownPlanningAndSurveyAndSharjahMunicipality(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await sharedRevenuesBetweenTheDirectorateOfTownPlanningAndSurveyAndSharjahMunicipality.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await sharedRevenuesBetweenTheDirectorateOfTownPlanningAndSurveyAndSharjahMunicipality.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await sharedRevenuesBetweenTheDirectorateOfTownPlanningAndSurveyAndSharjahMunicipality.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await sharedRevenuesBetweenTheDirectorateOfTownPlanningAndSurveyAndSharjahMunicipality.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await sharedRevenuesBetweenTheDirectorateOfTownPlanningAndSurveyAndSharjahMunicipality.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await sharedRevenuesBetweenTheDirectorateOfTownPlanningAndSurveyAndSharjahMunicipality.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await sharedRevenuesBetweenTheDirectorateOfTownPlanningAndSurveyAndSharjahMunicipality.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    sharedRevenuesBetweenTheDirectorateOfTownPlanningAndSurveyAndSharjahMunicipality.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await sharedRevenuesBetweenTheDirectorateOfTownPlanningAndSurveyAndSharjahMunicipality.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await sharedRevenuesBetweenTheDirectorateOfTownPlanningAndSurveyAndSharjahMunicipality.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await sharedRevenuesBetweenTheDirectorateOfTownPlanningAndSurveyAndSharjahMunicipality.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    sharedRevenuesBetweenTheDirectorateOfTownPlanningAndSurveyAndSharjahMunicipality.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    sharedRevenuesBetweenTheDirectorateOfTownPlanningAndSurveyAndSharjahMunicipality.selectors.submitButton as string
  );
});
