/**
 * Step Definitions for SFD Detailed Wallets Transactions Report Module
 *
 * This file contains all step definitions for the SFD Detailed Wallets Transactions Report feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module sfd-detailed-wallets-transactions-report.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { SfdDetailedWalletsTransactionsReport } from '../pages/generated/sfd-detailed-wallets-transactions-report.page';
 ...
 */
let page: Page;
let sfdDetailedWalletsTransactionsReport: SfdDetailedWalletsTransactionsReport;
...
function initializePageObject(): void {
  ...
  sfdDetailedWalletsTransactionsReport = new SfdDetailedWalletsTransactionsReport(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await sfdDetailedWalletsTransactionsReport.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await sfdDetailedWalletsTransactionsReport.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await sfdDetailedWalletsTransactionsReport.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await sfdDetailedWalletsTransactionsReport.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await sfdDetailedWalletsTransactionsReport.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await sfdDetailedWalletsTransactionsReport.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await sfdDetailedWalletsTransactionsReport.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    sfdDetailedWalletsTransactionsReport.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await sfdDetailedWalletsTransactionsReport.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await sfdDetailedWalletsTransactionsReport.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await sfdDetailedWalletsTransactionsReport.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    sfdDetailedWalletsTransactionsReport.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    sfdDetailedWalletsTransactionsReport.selectors.submitButton as string
  );
});
