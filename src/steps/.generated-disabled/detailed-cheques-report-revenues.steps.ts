/**
 * Step Definitions for Detailed Cheques Report - Revenues Module
 *
 * This file contains all step definitions for the Detailed Cheques Report - Revenues feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module detailed-cheques-report-revenues.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { DetailedChequesReportRevenues } from '../pages/generated/detailed-cheques-report-revenues.page';
 ...
 */
let page: Page;
let detailedChequesReportRevenues: DetailedChequesReportRevenues;
...
function initializePageObject(): void {
  ...
  detailedChequesReportRevenues = new DetailedChequesReportRevenues(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await detailedChequesReportRevenues.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await detailedChequesReportRevenues.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await detailedChequesReportRevenues.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await detailedChequesReportRevenues.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await detailedChequesReportRevenues.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await detailedChequesReportRevenues.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await detailedChequesReportRevenues.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    detailedChequesReportRevenues.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await detailedChequesReportRevenues.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await detailedChequesReportRevenues.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await detailedChequesReportRevenues.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    detailedChequesReportRevenues.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    detailedChequesReportRevenues.selectors.submitButton as string
  );
});
