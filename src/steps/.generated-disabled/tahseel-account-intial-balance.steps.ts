/**
 * Step Definitions for Tahseel Account Intial Balance Module
 *
 * This file contains all step definitions for the Tahseel Account Intial Balance feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module tahseel-account-intial-balance.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { TahseelAccountIntialBalance } from '../pages/generated/tahseel-account-intial-balance.page';
 ...
 */
let page: Page;
let tahseelAccountIntialBalance: TahseelAccountIntialBalance;
...
function initializePageObject(): void {
  ...
  tahseelAccountIntialBalance = new TahseelAccountIntialBalance(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await tahseelAccountIntialBalance.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await tahseelAccountIntialBalance.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await tahseelAccountIntialBalance.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await tahseelAccountIntialBalance.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await tahseelAccountIntialBalance.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await tahseelAccountIntialBalance.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await tahseelAccountIntialBalance.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    tahseelAccountIntialBalance.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await tahseelAccountIntialBalance.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await tahseelAccountIntialBalance.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await tahseelAccountIntialBalance.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    tahseelAccountIntialBalance.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    tahseelAccountIntialBalance.selectors.submitButton as string
  );
});
