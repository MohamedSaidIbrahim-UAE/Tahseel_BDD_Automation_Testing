/**
 * Step Definitions for Summary GITFees Report for smart Reciept Module
 *
 * This file contains all step definitions for the Summary GITFees Report for smart Reciept feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module summary-gitfees-report-for-smart-reciept.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { SummaryGitfeesReportForSmartReciept } from '../pages/generated/summary-gitfees-report-for-smart-reciept.page';
 ...
 */
let page: Page;
let summaryGitfeesReportForSmartReciept: SummaryGitfeesReportForSmartReciept;
...
function initializePageObject(): void {
  ...
  summaryGitfeesReportForSmartReciept = new SummaryGitfeesReportForSmartReciept(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await summaryGitfeesReportForSmartReciept.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await summaryGitfeesReportForSmartReciept.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await summaryGitfeesReportForSmartReciept.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await summaryGitfeesReportForSmartReciept.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await summaryGitfeesReportForSmartReciept.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await summaryGitfeesReportForSmartReciept.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await summaryGitfeesReportForSmartReciept.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    summaryGitfeesReportForSmartReciept.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await summaryGitfeesReportForSmartReciept.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await summaryGitfeesReportForSmartReciept.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await summaryGitfeesReportForSmartReciept.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    summaryGitfeesReportForSmartReciept.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    summaryGitfeesReportForSmartReciept.selectors.submitButton as string
  );
});
