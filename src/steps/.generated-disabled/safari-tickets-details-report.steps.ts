/**
 * Step Definitions for Safari Tickets Details Report Module
 *
 * This file contains all step definitions for the Safari Tickets Details Report feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module safari-tickets-details-report.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { SafariTicketsDetailsReport } from '../pages/generated/safari-tickets-details-report.page';
 ...
 */
let page: Page;
let safariTicketsDetailsReport: SafariTicketsDetailsReport;
...
function initializePageObject(): void {
  ...
  safariTicketsDetailsReport = new SafariTicketsDetailsReport(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await safariTicketsDetailsReport.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await safariTicketsDetailsReport.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await safariTicketsDetailsReport.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await safariTicketsDetailsReport.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await safariTicketsDetailsReport.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await safariTicketsDetailsReport.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await safariTicketsDetailsReport.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    safariTicketsDetailsReport.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await safariTicketsDetailsReport.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await safariTicketsDetailsReport.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await safariTicketsDetailsReport.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    safariTicketsDetailsReport.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    safariTicketsDetailsReport.selectors.submitButton as string
  );
});
