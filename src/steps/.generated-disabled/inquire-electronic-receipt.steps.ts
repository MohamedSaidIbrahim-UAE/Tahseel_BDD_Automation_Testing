/**
 * Step Definitions for Inquire Electronic Receipt Module
 *
 * This file contains all step definitions for the Inquire Electronic Receipt feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module inquire-electronic-receipt.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { InquireElectronicReceipt } from '../pages/generated/inquire-electronic-receipt.page';
 ...
 */
let page: Page;
let inquireElectronicReceipt: InquireElectronicReceipt;
...
function initializePageObject(): void {
  ...
  inquireElectronicReceipt = new InquireElectronicReceipt(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await inquireElectronicReceipt.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await inquireElectronicReceipt.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await inquireElectronicReceipt.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await inquireElectronicReceipt.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await inquireElectronicReceipt.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await inquireElectronicReceipt.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await inquireElectronicReceipt.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    inquireElectronicReceipt.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await inquireElectronicReceipt.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await inquireElectronicReceipt.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await inquireElectronicReceipt.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    inquireElectronicReceipt.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    inquireElectronicReceipt.selectors.submitButton as string
  );
});
