/**
 * Step Definitions for Grant Card History Logs Report Module
 *
 * This file contains all step definitions for the Grant Card History Logs Report feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module grant-card-history-logs-report.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { GrantCardHistoryLogsReport } from '../pages/generated/grant-card-history-logs-report.page';
 ...
 */
let page: Page;
let grantCardHistoryLogsReport: GrantCardHistoryLogsReport;
...
function initializePageObject(): void {
  ...
  grantCardHistoryLogsReport = new GrantCardHistoryLogsReport(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await grantCardHistoryLogsReport.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await grantCardHistoryLogsReport.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await grantCardHistoryLogsReport.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await grantCardHistoryLogsReport.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await grantCardHistoryLogsReport.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await grantCardHistoryLogsReport.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await grantCardHistoryLogsReport.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    grantCardHistoryLogsReport.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await grantCardHistoryLogsReport.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await grantCardHistoryLogsReport.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await grantCardHistoryLogsReport.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    grantCardHistoryLogsReport.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    grantCardHistoryLogsReport.selectors.submitButton as string
  );
});
