/**
 * Step Definitions for Dependant services revenue summary reports Module
 *
 * This file contains all step definitions for the Dependant services revenue summary reports feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module dependant-services-revenue-summary-reports.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { DependantServicesRevenueSummaryReports } from '../pages/generated/dependant-services-revenue-summary-reports.page';
 ...
 */
let page: Page;
let dependantServicesRevenueSummaryReports: DependantServicesRevenueSummaryReports;
...
function initializePageObject(): void {
  ...
  dependantServicesRevenueSummaryReports = new DependantServicesRevenueSummaryReports(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await dependantServicesRevenueSummaryReports.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await dependantServicesRevenueSummaryReports.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await dependantServicesRevenueSummaryReports.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await dependantServicesRevenueSummaryReports.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await dependantServicesRevenueSummaryReports.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await dependantServicesRevenueSummaryReports.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await dependantServicesRevenueSummaryReports.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    dependantServicesRevenueSummaryReports.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await dependantServicesRevenueSummaryReports.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await dependantServicesRevenueSummaryReports.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await dependantServicesRevenueSummaryReports.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    dependantServicesRevenueSummaryReports.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    dependantServicesRevenueSummaryReports.selectors.submitButton as string
  );
});
