/**
 * Step Definitions for Detailed Report of Smart Reciept Services Fees Module
 *
 * This file contains all step definitions for the Detailed Report of Smart Reciept Services Fees feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module detailed-report-of-smart-reciept-services-fees.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { DetailedReportOfSmartRecieptServicesFees } from '../pages/generated/detailed-report-of-smart-reciept-services-fees.page';
 ...
 */
let page: Page;
let detailedReportOfSmartRecieptServicesFees: DetailedReportOfSmartRecieptServicesFees;
...
function initializePageObject(): void {
  ...
  detailedReportOfSmartRecieptServicesFees = new DetailedReportOfSmartRecieptServicesFees(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await detailedReportOfSmartRecieptServicesFees.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await detailedReportOfSmartRecieptServicesFees.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await detailedReportOfSmartRecieptServicesFees.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await detailedReportOfSmartRecieptServicesFees.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await detailedReportOfSmartRecieptServicesFees.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await detailedReportOfSmartRecieptServicesFees.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await detailedReportOfSmartRecieptServicesFees.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    detailedReportOfSmartRecieptServicesFees.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await detailedReportOfSmartRecieptServicesFees.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await detailedReportOfSmartRecieptServicesFees.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await detailedReportOfSmartRecieptServicesFees.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    detailedReportOfSmartRecieptServicesFees.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    detailedReportOfSmartRecieptServicesFees.selectors.submitButton as string
  );
});
