/**
 * Step Definitions for Bank transfers of Sharjah Islamic Bank General Module
 *
 * This file contains all step definitions for the Bank transfers of Sharjah Islamic Bank General feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module bank-transfers-of-sharjah-islamic-bank-general.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { BankTransfersOfSharjahIslamicBankGeneral } from '../pages/generated/bank-transfers-of-sharjah-islamic-bank-general.page';
 ...
 */
let page: Page;
let bankTransfersOfSharjahIslamicBankGeneral: BankTransfersOfSharjahIslamicBankGeneral;
...
function initializePageObject(): void {
  ...
  bankTransfersOfSharjahIslamicBankGeneral = new BankTransfersOfSharjahIslamicBankGeneral(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await bankTransfersOfSharjahIslamicBankGeneral.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await bankTransfersOfSharjahIslamicBankGeneral.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await bankTransfersOfSharjahIslamicBankGeneral.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await bankTransfersOfSharjahIslamicBankGeneral.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await bankTransfersOfSharjahIslamicBankGeneral.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await bankTransfersOfSharjahIslamicBankGeneral.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await bankTransfersOfSharjahIslamicBankGeneral.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    bankTransfersOfSharjahIslamicBankGeneral.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await bankTransfersOfSharjahIslamicBankGeneral.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await bankTransfersOfSharjahIslamicBankGeneral.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await bankTransfersOfSharjahIslamicBankGeneral.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    bankTransfersOfSharjahIslamicBankGeneral.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    bankTransfersOfSharjahIslamicBankGeneral.selectors.submitButton as string
  );
});
