/**
 * Step Definitions for A detailed report for Loading using Al Thiqa Club employee card Module
 *
 * This file contains all step definitions for the A detailed report for Loading using Al Thiqa Club employee card feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module a-detailed-report-for-loading-using-al-thiqa-club-employee-card.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { ADetailedReportForLoadingUsingAlThiqaClubEmployeeCard } from '../pages/generated/a-detailed-report-for-loading-using-al-thiqa-club-employee-card.page';
 ...
 */
let page: Page;
let aDetailedReportForLoadingUsingAlThiqaClubEmployeeCard: ADetailedReportForLoadingUsingAlThiqaClubEmployeeCard;
...
function initializePageObject(): void {
  ...
  aDetailedReportForLoadingUsingAlThiqaClubEmployeeCard = new ADetailedReportForLoadingUsingAlThiqaClubEmployeeCard(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await aDetailedReportForLoadingUsingAlThiqaClubEmployeeCard.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await aDetailedReportForLoadingUsingAlThiqaClubEmployeeCard.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await aDetailedReportForLoadingUsingAlThiqaClubEmployeeCard.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await aDetailedReportForLoadingUsingAlThiqaClubEmployeeCard.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await aDetailedReportForLoadingUsingAlThiqaClubEmployeeCard.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await aDetailedReportForLoadingUsingAlThiqaClubEmployeeCard.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await aDetailedReportForLoadingUsingAlThiqaClubEmployeeCard.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    aDetailedReportForLoadingUsingAlThiqaClubEmployeeCard.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await aDetailedReportForLoadingUsingAlThiqaClubEmployeeCard.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await aDetailedReportForLoadingUsingAlThiqaClubEmployeeCard.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await aDetailedReportForLoadingUsingAlThiqaClubEmployeeCard.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    aDetailedReportForLoadingUsingAlThiqaClubEmployeeCard.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    aDetailedReportForLoadingUsingAlThiqaClubEmployeeCard.selectors.submitButton as string
  );
});
