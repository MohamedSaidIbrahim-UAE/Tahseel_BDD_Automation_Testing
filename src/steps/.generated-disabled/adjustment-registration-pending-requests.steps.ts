/**
 * Step Definitions for Adjustment Registration Pending Requests Module
 *
 * This file contains all step definitions for the Adjustment Registration Pending Requests feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module adjustment-registration-pending-requests.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { AdjustmentRegistrationPendingRequests } from '../pages/generated/adjustment-registration-pending-requests.page';
 ...
 */
let page: Page;
let adjustmentRegistrationPendingRequests: AdjustmentRegistrationPendingRequests;
...
function initializePageObject(): void {
  ...
  adjustmentRegistrationPendingRequests = new AdjustmentRegistrationPendingRequests(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await adjustmentRegistrationPendingRequests.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await adjustmentRegistrationPendingRequests.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await adjustmentRegistrationPendingRequests.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await adjustmentRegistrationPendingRequests.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await adjustmentRegistrationPendingRequests.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await adjustmentRegistrationPendingRequests.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await adjustmentRegistrationPendingRequests.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    adjustmentRegistrationPendingRequests.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await adjustmentRegistrationPendingRequests.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await adjustmentRegistrationPendingRequests.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await adjustmentRegistrationPendingRequests.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    adjustmentRegistrationPendingRequests.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    adjustmentRegistrationPendingRequests.selectors.submitButton as string
  );
});
