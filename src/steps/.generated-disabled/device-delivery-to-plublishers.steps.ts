/**
 * Step Definitions for Device Delivery To Plublishers Module
 *
 * This file contains all step definitions for the Device Delivery To Plublishers feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module device-delivery-to-plublishers.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { DeviceDeliveryToPlublishers } from '../pages/generated/device-delivery-to-plublishers.page';
 ...
 */
let page: Page;
let deviceDeliveryToPlublishers: DeviceDeliveryToPlublishers;
...
function initializePageObject(): void {
  ...
  deviceDeliveryToPlublishers = new DeviceDeliveryToPlublishers(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await deviceDeliveryToPlublishers.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await deviceDeliveryToPlublishers.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await deviceDeliveryToPlublishers.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await deviceDeliveryToPlublishers.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await deviceDeliveryToPlublishers.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await deviceDeliveryToPlublishers.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await deviceDeliveryToPlublishers.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    deviceDeliveryToPlublishers.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await deviceDeliveryToPlublishers.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await deviceDeliveryToPlublishers.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await deviceDeliveryToPlublishers.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    deviceDeliveryToPlublishers.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    deviceDeliveryToPlublishers.selectors.submitButton as string
  );
});
