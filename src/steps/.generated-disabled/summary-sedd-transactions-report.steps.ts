/**
 * Step Definitions for Summary SEDD Transactions Report Module
 *
 * This file contains all step definitions for the Summary SEDD Transactions Report feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module summary-sedd-transactions-report.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { SummarySeddTransactionsReport } from '../pages/generated/summary-sedd-transactions-report.page';
 ...
 */
let page: Page;
let summarySeddTransactionsReport: SummarySeddTransactionsReport;
...
function initializePageObject(): void {
  ...
  summarySeddTransactionsReport = new SummarySeddTransactionsReport(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await summarySeddTransactionsReport.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await summarySeddTransactionsReport.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await summarySeddTransactionsReport.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await summarySeddTransactionsReport.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await summarySeddTransactionsReport.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await summarySeddTransactionsReport.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await summarySeddTransactionsReport.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    summarySeddTransactionsReport.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await summarySeddTransactionsReport.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await summarySeddTransactionsReport.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await summarySeddTransactionsReport.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    summarySeddTransactionsReport.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    summarySeddTransactionsReport.selectors.submitButton as string
  );
});
