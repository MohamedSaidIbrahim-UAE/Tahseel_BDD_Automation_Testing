/**
 * Step Definitions for Daily Users Loading Report For Tasheel Centers Module
 *
 * This file contains all step definitions for the Daily Users Loading Report For Tasheel Centers feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module daily-users-loading-report-for-tasheel-centers.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { DailyUsersLoadingReportForTasheelCenters } from '../pages/generated/daily-users-loading-report-for-tasheel-centers.page';
 ...
 */
let page: Page;
let dailyUsersLoadingReportForTasheelCenters: DailyUsersLoadingReportForTasheelCenters;
...
function initializePageObject(): void {
  ...
  dailyUsersLoadingReportForTasheelCenters = new DailyUsersLoadingReportForTasheelCenters(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await dailyUsersLoadingReportForTasheelCenters.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await dailyUsersLoadingReportForTasheelCenters.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await dailyUsersLoadingReportForTasheelCenters.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await dailyUsersLoadingReportForTasheelCenters.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await dailyUsersLoadingReportForTasheelCenters.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await dailyUsersLoadingReportForTasheelCenters.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await dailyUsersLoadingReportForTasheelCenters.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    dailyUsersLoadingReportForTasheelCenters.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await dailyUsersLoadingReportForTasheelCenters.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await dailyUsersLoadingReportForTasheelCenters.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await dailyUsersLoadingReportForTasheelCenters.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    dailyUsersLoadingReportForTasheelCenters.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    dailyUsersLoadingReportForTasheelCenters.selectors.submitButton as string
  );
});
