/**
 * Step Definitions for Manage Safari Booking Dates Module
 *
 * This file contains all step definitions for the Manage Safari Booking Dates feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module manage-safari-booking-dates.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { ManageSafariBookingDates } from '../pages/generated/manage-safari-booking-dates.page';
 ...
 */
let page: Page;
let manageSafariBookingDates: ManageSafariBookingDates;
...
function initializePageObject(): void {
  ...
  manageSafariBookingDates = new ManageSafariBookingDates(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await manageSafariBookingDates.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await manageSafariBookingDates.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await manageSafariBookingDates.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await manageSafariBookingDates.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await manageSafariBookingDates.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await manageSafariBookingDates.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await manageSafariBookingDates.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    manageSafariBookingDates.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await manageSafariBookingDates.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await manageSafariBookingDates.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await manageSafariBookingDates.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    manageSafariBookingDates.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    manageSafariBookingDates.selectors.submitButton as string
  );
});
