/**
 * Step Definitions for Aggregated fees Report Tahseel Cards Module
 *
 * This file contains all step definitions for the Aggregated fees Report Tahseel Cards feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module aggregated-fees-report-tahseel-cards.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { AggregatedFeesReportTahseelCards } from '../pages/generated/aggregated-fees-report-tahseel-cards.page';
 ...
 */
let page: Page;
let aggregatedFeesReportTahseelCards: AggregatedFeesReportTahseelCards;
...
function initializePageObject(): void {
  ...
  aggregatedFeesReportTahseelCards = new AggregatedFeesReportTahseelCards(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await aggregatedFeesReportTahseelCards.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await aggregatedFeesReportTahseelCards.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await aggregatedFeesReportTahseelCards.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await aggregatedFeesReportTahseelCards.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await aggregatedFeesReportTahseelCards.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await aggregatedFeesReportTahseelCards.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await aggregatedFeesReportTahseelCards.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    aggregatedFeesReportTahseelCards.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await aggregatedFeesReportTahseelCards.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await aggregatedFeesReportTahseelCards.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await aggregatedFeesReportTahseelCards.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    aggregatedFeesReportTahseelCards.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    aggregatedFeesReportTahseelCards.selectors.submitButton as string
  );
});
