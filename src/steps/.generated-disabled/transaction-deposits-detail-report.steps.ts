/**
 * Step Definitions for Transaction deposits detail Report Module
 *
 * This file contains all step definitions for the Transaction deposits detail Report feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module transaction-deposits-detail-report.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { TransactionDepositsDetailReport } from '../pages/generated/transaction-deposits-detail-report.page';
 ...
 */
let page: Page;
let transactionDepositsDetailReport: TransactionDepositsDetailReport;
...
function initializePageObject(): void {
  ...
  transactionDepositsDetailReport = new TransactionDepositsDetailReport(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await transactionDepositsDetailReport.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await transactionDepositsDetailReport.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await transactionDepositsDetailReport.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await transactionDepositsDetailReport.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await transactionDepositsDetailReport.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await transactionDepositsDetailReport.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await transactionDepositsDetailReport.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    transactionDepositsDetailReport.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await transactionDepositsDetailReport.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await transactionDepositsDetailReport.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await transactionDepositsDetailReport.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    transactionDepositsDetailReport.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    transactionDepositsDetailReport.selectors.submitButton as string
  );
});
