/**
 * Step Definitions for Tahseel Card Transaction Details Reports Module
 *
 * This file contains all step definitions for the Tahseel Card Transaction Details Reports feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module tahseel-card-transaction-details-reports.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { TahseelCardTransactionDetailsReports } from '../pages/generated/tahseel-card-transaction-details-reports.page';
 ...
 */
let page: Page;
let tahseelCardTransactionDetailsReports: TahseelCardTransactionDetailsReports;
...
function initializePageObject(): void {
  ...
  tahseelCardTransactionDetailsReports = new TahseelCardTransactionDetailsReports(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await tahseelCardTransactionDetailsReports.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await tahseelCardTransactionDetailsReports.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await tahseelCardTransactionDetailsReports.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await tahseelCardTransactionDetailsReports.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await tahseelCardTransactionDetailsReports.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await tahseelCardTransactionDetailsReports.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await tahseelCardTransactionDetailsReports.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    tahseelCardTransactionDetailsReports.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await tahseelCardTransactionDetailsReports.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await tahseelCardTransactionDetailsReports.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await tahseelCardTransactionDetailsReports.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    tahseelCardTransactionDetailsReports.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    tahseelCardTransactionDetailsReports.selectors.submitButton as string
  );
});
