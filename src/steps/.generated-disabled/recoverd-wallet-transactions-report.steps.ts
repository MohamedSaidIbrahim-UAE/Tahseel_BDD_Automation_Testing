/**
 * Step Definitions for Recoverd Wallet Transactions Report Module
 *
 * This file contains all step definitions for the Recoverd Wallet Transactions Report feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module recoverd-wallet-transactions-report.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { RecoverdWalletTransactionsReport } from '../pages/generated/recoverd-wallet-transactions-report.page';
 ...
 */
let page: Page;
let recoverdWalletTransactionsReport: RecoverdWalletTransactionsReport;
...
function initializePageObject(): void {
  ...
  recoverdWalletTransactionsReport = new RecoverdWalletTransactionsReport(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await recoverdWalletTransactionsReport.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await recoverdWalletTransactionsReport.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await recoverdWalletTransactionsReport.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await recoverdWalletTransactionsReport.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await recoverdWalletTransactionsReport.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await recoverdWalletTransactionsReport.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await recoverdWalletTransactionsReport.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    recoverdWalletTransactionsReport.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await recoverdWalletTransactionsReport.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await recoverdWalletTransactionsReport.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await recoverdWalletTransactionsReport.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    recoverdWalletTransactionsReport.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    recoverdWalletTransactionsReport.selectors.submitButton as string
  );
});
