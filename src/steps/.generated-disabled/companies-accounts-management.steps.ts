/**
 * Step Definitions for Companies Accounts Management Module
 *
 * This file contains all step definitions for the Companies Accounts Management feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module companies-accounts-management.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { CompaniesAccountsManagement } from '../pages/generated/companies-accounts-management.page';
 ...
 */
let page: Page;
let companiesAccountsManagement: CompaniesAccountsManagement;
...
function initializePageObject(): void {
  ...
  companiesAccountsManagement = new CompaniesAccountsManagement(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await companiesAccountsManagement.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await companiesAccountsManagement.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await companiesAccountsManagement.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await companiesAccountsManagement.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await companiesAccountsManagement.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await companiesAccountsManagement.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await companiesAccountsManagement.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    companiesAccountsManagement.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await companiesAccountsManagement.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await companiesAccountsManagement.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await companiesAccountsManagement.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    companiesAccountsManagement.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    companiesAccountsManagement.selectors.submitButton as string
  );
});
