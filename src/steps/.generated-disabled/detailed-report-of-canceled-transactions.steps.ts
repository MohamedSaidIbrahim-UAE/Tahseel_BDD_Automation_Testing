/**
 * Step Definitions for Detailed report of canceled transactions Module
 *
 * This file contains all step definitions for the Detailed report of canceled transactions feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module detailed-report-of-canceled-transactions.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { DetailedReportOfCanceledTransactions } from '../pages/generated/detailed-report-of-canceled-transactions.page';
 ...
 */
let page: Page;
let detailedReportOfCanceledTransactions: DetailedReportOfCanceledTransactions;
...
function initializePageObject(): void {
  ...
  detailedReportOfCanceledTransactions = new DetailedReportOfCanceledTransactions(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await detailedReportOfCanceledTransactions.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await detailedReportOfCanceledTransactions.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await detailedReportOfCanceledTransactions.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await detailedReportOfCanceledTransactions.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await detailedReportOfCanceledTransactions.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await detailedReportOfCanceledTransactions.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await detailedReportOfCanceledTransactions.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    detailedReportOfCanceledTransactions.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await detailedReportOfCanceledTransactions.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await detailedReportOfCanceledTransactions.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await detailedReportOfCanceledTransactions.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    detailedReportOfCanceledTransactions.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    detailedReportOfCanceledTransactions.selectors.submitButton as string
  );
});
