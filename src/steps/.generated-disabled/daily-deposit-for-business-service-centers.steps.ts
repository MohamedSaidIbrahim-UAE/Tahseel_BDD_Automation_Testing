/**
 * Step Definitions for Daily deposit for Business Service Centers Module
 *
 * This file contains all step definitions for the Daily deposit for Business Service Centers feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module daily-deposit-for-business-service-centers.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { DailyDepositForBusinessServiceCenters } from '../pages/generated/daily-deposit-for-business-service-centers.page';
 ...
 */
let page: Page;
let dailyDepositForBusinessServiceCenters: DailyDepositForBusinessServiceCenters;
...
function initializePageObject(): void {
  ...
  dailyDepositForBusinessServiceCenters = new DailyDepositForBusinessServiceCenters(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await dailyDepositForBusinessServiceCenters.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await dailyDepositForBusinessServiceCenters.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await dailyDepositForBusinessServiceCenters.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await dailyDepositForBusinessServiceCenters.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await dailyDepositForBusinessServiceCenters.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await dailyDepositForBusinessServiceCenters.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await dailyDepositForBusinessServiceCenters.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    dailyDepositForBusinessServiceCenters.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await dailyDepositForBusinessServiceCenters.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await dailyDepositForBusinessServiceCenters.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await dailyDepositForBusinessServiceCenters.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    dailyDepositForBusinessServiceCenters.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    dailyDepositForBusinessServiceCenters.selectors.submitButton as string
  );
});
