/**
 * Step Definitions for Sharjah islamic bank settlement report Module
 *
 * This file contains all step definitions for the Sharjah islamic bank settlement report feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module sharjah-islamic-bank-settlement-report.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { SharjahIslamicBankSettlementReport } from '../pages/generated/sharjah-islamic-bank-settlement-report.page';
 ...
 */
let page: Page;
let sharjahIslamicBankSettlementReport: SharjahIslamicBankSettlementReport;
...
function initializePageObject(): void {
  ...
  sharjahIslamicBankSettlementReport = new SharjahIslamicBankSettlementReport(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await sharjahIslamicBankSettlementReport.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await sharjahIslamicBankSettlementReport.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await sharjahIslamicBankSettlementReport.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await sharjahIslamicBankSettlementReport.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await sharjahIslamicBankSettlementReport.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await sharjahIslamicBankSettlementReport.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await sharjahIslamicBankSettlementReport.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    sharjahIslamicBankSettlementReport.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await sharjahIslamicBankSettlementReport.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await sharjahIslamicBankSettlementReport.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await sharjahIslamicBankSettlementReport.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    sharjahIslamicBankSettlementReport.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    sharjahIslamicBankSettlementReport.selectors.submitButton as string
  );
});
