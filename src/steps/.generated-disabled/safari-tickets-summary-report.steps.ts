/**
 * Step Definitions for Safari Tickets Summary Report Module
 *
 * This file contains all step definitions for the Safari Tickets Summary Report feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module safari-tickets-summary-report.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { SafariTicketsSummaryReport } from '../pages/generated/safari-tickets-summary-report.page';
 ...
 */
let page: Page;
let safariTicketsSummaryReport: SafariTicketsSummaryReport;
...
function initializePageObject(): void {
  ...
  safariTicketsSummaryReport = new SafariTicketsSummaryReport(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await safariTicketsSummaryReport.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await safariTicketsSummaryReport.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await safariTicketsSummaryReport.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await safariTicketsSummaryReport.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await safariTicketsSummaryReport.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await safariTicketsSummaryReport.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await safariTicketsSummaryReport.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    safariTicketsSummaryReport.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await safariTicketsSummaryReport.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await safariTicketsSummaryReport.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await safariTicketsSummaryReport.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    safariTicketsSummaryReport.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    safariTicketsSummaryReport.selectors.submitButton as string
  );
});
