/**
 * Step Definitions for Detailed Transactions Report Module
 *
 * This file contains all step definitions for the Detailed Transactions Report feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module detailed-transactions-report.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { DetailedTransactionsReport } from '../pages/generated/detailed-transactions-report.page';
 ...
 */
let page: Page;
let detailedTransactionsReport: DetailedTransactionsReport;
...
function initializePageObject(): void {
  ...
  detailedTransactionsReport = new DetailedTransactionsReport(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await detailedTransactionsReport.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await detailedTransactionsReport.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await detailedTransactionsReport.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await detailedTransactionsReport.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await detailedTransactionsReport.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await detailedTransactionsReport.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await detailedTransactionsReport.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    detailedTransactionsReport.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await detailedTransactionsReport.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await detailedTransactionsReport.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await detailedTransactionsReport.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    detailedTransactionsReport.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    detailedTransactionsReport.selectors.submitButton as string
  );
});
