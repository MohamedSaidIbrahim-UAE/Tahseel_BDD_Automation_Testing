/**
 * Step Definitions for Cancelled Refund Requests Module
 *
 * This file contains all step definitions for the Cancelled Refund Requests feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module cancelled-refund-requests.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { CancelledRefundRequests } from '../pages/generated/cancelled-refund-requests.page';
 ...
 */
let page: Page;
let cancelledRefundRequests: CancelledRefundRequests;
...
function initializePageObject(): void {
  ...
  cancelledRefundRequests = new CancelledRefundRequests(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await cancelledRefundRequests.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await cancelledRefundRequests.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await cancelledRefundRequests.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await cancelledRefundRequests.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await cancelledRefundRequests.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await cancelledRefundRequests.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await cancelledRefundRequests.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    cancelledRefundRequests.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await cancelledRefundRequests.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await cancelledRefundRequests.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await cancelledRefundRequests.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    cancelledRefundRequests.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    cancelledRefundRequests.selectors.submitButton as string
  );
});
