/**
 * Step Definitions for Mashreq bank settlement report in period Module
 *
 * This file contains all step definitions for the Mashreq bank settlement report in period feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module mashreq-bank-settlement-report-in-period.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { MashreqBankSettlementReportInPeriod } from '../pages/generated/mashreq-bank-settlement-report-in-period.page';
 ...
 */
let page: Page;
let mashreqBankSettlementReportInPeriod: MashreqBankSettlementReportInPeriod;
...
function initializePageObject(): void {
  ...
  mashreqBankSettlementReportInPeriod = new MashreqBankSettlementReportInPeriod(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await mashreqBankSettlementReportInPeriod.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await mashreqBankSettlementReportInPeriod.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await mashreqBankSettlementReportInPeriod.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await mashreqBankSettlementReportInPeriod.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await mashreqBankSettlementReportInPeriod.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await mashreqBankSettlementReportInPeriod.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await mashreqBankSettlementReportInPeriod.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    mashreqBankSettlementReportInPeriod.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await mashreqBankSettlementReportInPeriod.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await mashreqBankSettlementReportInPeriod.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await mashreqBankSettlementReportInPeriod.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    mashreqBankSettlementReportInPeriod.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    mashreqBankSettlementReportInPeriod.selectors.submitButton as string
  );
});
