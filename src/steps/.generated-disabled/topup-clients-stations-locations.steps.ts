/**
 * Step Definitions for Topup Clients Stations Locations Module
 *
 * This file contains all step definitions for the Topup Clients Stations Locations feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module topup-clients-stations-locations.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { TopupClientsStationsLocations } from '../pages/generated/topup-clients-stations-locations.page';
 ...
 */
let page: Page;
let topupClientsStationsLocations: TopupClientsStationsLocations;
...
function initializePageObject(): void {
  ...
  topupClientsStationsLocations = new TopupClientsStationsLocations(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await topupClientsStationsLocations.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await topupClientsStationsLocations.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await topupClientsStationsLocations.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await topupClientsStationsLocations.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await topupClientsStationsLocations.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await topupClientsStationsLocations.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await topupClientsStationsLocations.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    topupClientsStationsLocations.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await topupClientsStationsLocations.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await topupClientsStationsLocations.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await topupClientsStationsLocations.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    topupClientsStationsLocations.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    topupClientsStationsLocations.selectors.submitButton as string
  );
});
