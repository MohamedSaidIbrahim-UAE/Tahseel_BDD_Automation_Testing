/**
 * Step Definitions for Credit Card Summery By Merchant Module
 *
 * This file contains all step definitions for the Credit Card Summery By Merchant feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module credit-card-summery-by-merchant.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { CreditCardSummeryByMerchant } from '../pages/generated/credit-card-summery-by-merchant.page';
 ...
 */
let page: Page;
let creditCardSummeryByMerchant: CreditCardSummeryByMerchant;
...
function initializePageObject(): void {
  ...
  creditCardSummeryByMerchant = new CreditCardSummeryByMerchant(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await creditCardSummeryByMerchant.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await creditCardSummeryByMerchant.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await creditCardSummeryByMerchant.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await creditCardSummeryByMerchant.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await creditCardSummeryByMerchant.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await creditCardSummeryByMerchant.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await creditCardSummeryByMerchant.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    creditCardSummeryByMerchant.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await creditCardSummeryByMerchant.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await creditCardSummeryByMerchant.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await creditCardSummeryByMerchant.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    creditCardSummeryByMerchant.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    creditCardSummeryByMerchant.selectors.submitButton as string
  );
});
