/**
 * Step Definitions for E-Voucher Report Transaction Module
 *
 * This file contains all step definitions for the E-Voucher Report Transaction feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module e-voucher-report-transaction.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { EVoucherReportTransaction } from '../pages/generated/e-voucher-report-transaction.page';
 ...
 */
let page: Page;
let eVoucherReportTransaction: EVoucherReportTransaction;
...
function initializePageObject(): void {
  ...
  eVoucherReportTransaction = new EVoucherReportTransaction(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await eVoucherReportTransaction.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await eVoucherReportTransaction.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await eVoucherReportTransaction.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await eVoucherReportTransaction.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await eVoucherReportTransaction.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await eVoucherReportTransaction.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await eVoucherReportTransaction.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    eVoucherReportTransaction.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await eVoucherReportTransaction.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await eVoucherReportTransaction.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await eVoucherReportTransaction.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    eVoucherReportTransaction.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    eVoucherReportTransaction.selectors.submitButton as string
  );
});
