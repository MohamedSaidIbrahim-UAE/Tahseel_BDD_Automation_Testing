/**
 * Step Definitions for Transaction Done by TahseelCard Module
 *
 * This file contains all step definitions for the Transaction Done by TahseelCard feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module transaction-done-by-tahseelcard.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { TransactionDoneByTahseelcard } from '../pages/generated/transaction-done-by-tahseelcard.page';
 ...
 */
let page: Page;
let transactionDoneByTahseelcard: TransactionDoneByTahseelcard;
...
function initializePageObject(): void {
  ...
  transactionDoneByTahseelcard = new TransactionDoneByTahseelcard(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await transactionDoneByTahseelcard.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await transactionDoneByTahseelcard.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await transactionDoneByTahseelcard.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await transactionDoneByTahseelcard.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await transactionDoneByTahseelcard.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await transactionDoneByTahseelcard.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await transactionDoneByTahseelcard.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    transactionDoneByTahseelcard.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await transactionDoneByTahseelcard.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await transactionDoneByTahseelcard.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await transactionDoneByTahseelcard.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    transactionDoneByTahseelcard.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    transactionDoneByTahseelcard.selectors.submitButton as string
  );
});
