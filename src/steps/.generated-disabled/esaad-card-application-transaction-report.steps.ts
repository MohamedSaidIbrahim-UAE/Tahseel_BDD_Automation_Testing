/**
 * Step Definitions for Esaad card application transaction report Module
 *
 * This file contains all step definitions for the Esaad card application transaction report feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module esaad-card-application-transaction-report.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { EsaadCardApplicationTransactionReport } from '../pages/generated/esaad-card-application-transaction-report.page';
 ...
 */
let page: Page;
let esaadCardApplicationTransactionReport: EsaadCardApplicationTransactionReport;
...
function initializePageObject(): void {
  ...
  esaadCardApplicationTransactionReport = new EsaadCardApplicationTransactionReport(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await esaadCardApplicationTransactionReport.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await esaadCardApplicationTransactionReport.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await esaadCardApplicationTransactionReport.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await esaadCardApplicationTransactionReport.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await esaadCardApplicationTransactionReport.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await esaadCardApplicationTransactionReport.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await esaadCardApplicationTransactionReport.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    esaadCardApplicationTransactionReport.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await esaadCardApplicationTransactionReport.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await esaadCardApplicationTransactionReport.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await esaadCardApplicationTransactionReport.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    esaadCardApplicationTransactionReport.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    esaadCardApplicationTransactionReport.selectors.submitButton as string
  );
});
