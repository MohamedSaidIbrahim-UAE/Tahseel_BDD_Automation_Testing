/**
 * Step Definitions for Deposit Retain Requests Inquiry Module
 *
 * This file contains all step definitions for the Deposit Retain Requests Inquiry feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module deposit-retain-requests-inquiry.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { DepositRetainRequestsInquiry } from '../pages/generated/deposit-retain-requests-inquiry.page';
 ...
 */
let page: Page;
let depositRetainRequestsInquiry: DepositRetainRequestsInquiry;
...
function initializePageObject(): void {
  ...
  depositRetainRequestsInquiry = new DepositRetainRequestsInquiry(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await depositRetainRequestsInquiry.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await depositRetainRequestsInquiry.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await depositRetainRequestsInquiry.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await depositRetainRequestsInquiry.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await depositRetainRequestsInquiry.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await depositRetainRequestsInquiry.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await depositRetainRequestsInquiry.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    depositRetainRequestsInquiry.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await depositRetainRequestsInquiry.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await depositRetainRequestsInquiry.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await depositRetainRequestsInquiry.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    depositRetainRequestsInquiry.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    depositRetainRequestsInquiry.selectors.submitButton as string
  );
});
