/**
 * Step Definitions for Aggregated Loading Report of Evouchers Module
 *
 * This file contains all step definitions for the Aggregated Loading Report of Evouchers feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module aggregated-loading-report-of-evouchers.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { AggregatedLoadingReportOfEvouchers } from '../pages/generated/aggregated-loading-report-of-evouchers.page';
 ...
 */
let page: Page;
let aggregatedLoadingReportOfEvouchers: AggregatedLoadingReportOfEvouchers;
...
function initializePageObject(): void {
  ...
  aggregatedLoadingReportOfEvouchers = new AggregatedLoadingReportOfEvouchers(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await aggregatedLoadingReportOfEvouchers.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await aggregatedLoadingReportOfEvouchers.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await aggregatedLoadingReportOfEvouchers.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await aggregatedLoadingReportOfEvouchers.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await aggregatedLoadingReportOfEvouchers.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await aggregatedLoadingReportOfEvouchers.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await aggregatedLoadingReportOfEvouchers.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    aggregatedLoadingReportOfEvouchers.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await aggregatedLoadingReportOfEvouchers.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await aggregatedLoadingReportOfEvouchers.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await aggregatedLoadingReportOfEvouchers.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    aggregatedLoadingReportOfEvouchers.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    aggregatedLoadingReportOfEvouchers.selectors.submitButton as string
  );
});
