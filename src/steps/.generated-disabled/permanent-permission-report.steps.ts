/**
 * Step Definitions for Permanent permission report Module
 *
 * This file contains all step definitions for the Permanent permission report feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module permanent-permission-report.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { PermanentPermissionReport } from '../pages/generated/permanent-permission-report.page';
 ...
 */
let page: Page;
let permanentPermissionReport: PermanentPermissionReport;
...
function initializePageObject(): void {
  ...
  permanentPermissionReport = new PermanentPermissionReport(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await permanentPermissionReport.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await permanentPermissionReport.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await permanentPermissionReport.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await permanentPermissionReport.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await permanentPermissionReport.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await permanentPermissionReport.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await permanentPermissionReport.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    permanentPermissionReport.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await permanentPermissionReport.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await permanentPermissionReport.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await permanentPermissionReport.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    permanentPermissionReport.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    permanentPermissionReport.selectors.submitButton as string
  );
});
