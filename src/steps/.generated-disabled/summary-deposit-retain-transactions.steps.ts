/**
 * Step Definitions for Summary Deposit Retain Transactions Module
 *
 * This file contains all step definitions for the Summary Deposit Retain Transactions feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module summary-deposit-retain-transactions.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { SummaryDepositRetainTransactions } from '../pages/generated/summary-deposit-retain-transactions.page';
 ...
 */
let page: Page;
let summaryDepositRetainTransactions: SummaryDepositRetainTransactions;
...
function initializePageObject(): void {
  ...
  summaryDepositRetainTransactions = new SummaryDepositRetainTransactions(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await summaryDepositRetainTransactions.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await summaryDepositRetainTransactions.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await summaryDepositRetainTransactions.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await summaryDepositRetainTransactions.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await summaryDepositRetainTransactions.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await summaryDepositRetainTransactions.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await summaryDepositRetainTransactions.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    summaryDepositRetainTransactions.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await summaryDepositRetainTransactions.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await summaryDepositRetainTransactions.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await summaryDepositRetainTransactions.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    summaryDepositRetainTransactions.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    summaryDepositRetainTransactions.selectors.submitButton as string
  );
});
