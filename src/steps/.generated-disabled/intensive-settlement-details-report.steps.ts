/**
 * Step Definitions for Intensive Settlement Details Report Module
 *
 * This file contains all step definitions for the Intensive Settlement Details Report feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module intensive-settlement-details-report.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { IntensiveSettlementDetailsReport } from '../pages/generated/intensive-settlement-details-report.page';
 ...
 */
let page: Page;
let intensiveSettlementDetailsReport: IntensiveSettlementDetailsReport;
...
function initializePageObject(): void {
  ...
  intensiveSettlementDetailsReport = new IntensiveSettlementDetailsReport(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await intensiveSettlementDetailsReport.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await intensiveSettlementDetailsReport.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await intensiveSettlementDetailsReport.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await intensiveSettlementDetailsReport.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await intensiveSettlementDetailsReport.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await intensiveSettlementDetailsReport.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await intensiveSettlementDetailsReport.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    intensiveSettlementDetailsReport.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await intensiveSettlementDetailsReport.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await intensiveSettlementDetailsReport.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await intensiveSettlementDetailsReport.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    intensiveSettlementDetailsReport.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    intensiveSettlementDetailsReport.selectors.submitButton as string
  );
});
