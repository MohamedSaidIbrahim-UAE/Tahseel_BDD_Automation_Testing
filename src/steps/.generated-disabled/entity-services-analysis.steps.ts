/**
 * Step Definitions for Entity Services Analysis Module
 *
 * This file contains all step definitions for the Entity Services Analysis feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module entity-services-analysis.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { EntityServicesAnalysis } from '../pages/generated/entity-services-analysis.page';
 ...
 */
let page: Page;
let entityServicesAnalysis: EntityServicesAnalysis;
...
function initializePageObject(): void {
  ...
  entityServicesAnalysis = new EntityServicesAnalysis(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await entityServicesAnalysis.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await entityServicesAnalysis.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await entityServicesAnalysis.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await entityServicesAnalysis.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await entityServicesAnalysis.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await entityServicesAnalysis.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await entityServicesAnalysis.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    entityServicesAnalysis.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await entityServicesAnalysis.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await entityServicesAnalysis.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await entityServicesAnalysis.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    entityServicesAnalysis.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    entityServicesAnalysis.selectors.submitButton as string
  );
});
