/**
 * Step Definitions for Etisalat detailed transactions from Digital Sharjah Module
 *
 * This file contains all step definitions for the Etisalat detailed transactions from Digital Sharjah feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module etisalat-detailed-transactions-from-digital-sharjah.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { EtisalatDetailedTransactionsFromDigitalSharjah } from '../pages/generated/etisalat-detailed-transactions-from-digital-sharjah.page';
 ...
 */
let page: Page;
let etisalatDetailedTransactionsFromDigitalSharjah: EtisalatDetailedTransactionsFromDigitalSharjah;
...
function initializePageObject(): void {
  ...
  etisalatDetailedTransactionsFromDigitalSharjah = new EtisalatDetailedTransactionsFromDigitalSharjah(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await etisalatDetailedTransactionsFromDigitalSharjah.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await etisalatDetailedTransactionsFromDigitalSharjah.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await etisalatDetailedTransactionsFromDigitalSharjah.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await etisalatDetailedTransactionsFromDigitalSharjah.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await etisalatDetailedTransactionsFromDigitalSharjah.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await etisalatDetailedTransactionsFromDigitalSharjah.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await etisalatDetailedTransactionsFromDigitalSharjah.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    etisalatDetailedTransactionsFromDigitalSharjah.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await etisalatDetailedTransactionsFromDigitalSharjah.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await etisalatDetailedTransactionsFromDigitalSharjah.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await etisalatDetailedTransactionsFromDigitalSharjah.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    etisalatDetailedTransactionsFromDigitalSharjah.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    etisalatDetailedTransactionsFromDigitalSharjah.selectors.submitButton as string
  );
});
