/**
 * Step Definitions for Bank transfers of Sharjah Islamic Bank Module
 *
 * This file contains all step definitions for the Bank transfers of Sharjah Islamic Bank feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module bank-transfers-of-sharjah-islamic-bank.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { BankTransfersOfSharjahIslamicBank } from '../pages/generated/bank-transfers-of-sharjah-islamic-bank.page';
 ...
 */
let page: Page;
let bankTransfersOfSharjahIslamicBank: BankTransfersOfSharjahIslamicBank;
...
function initializePageObject(): void {
  ...
  bankTransfersOfSharjahIslamicBank = new BankTransfersOfSharjahIslamicBank(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await bankTransfersOfSharjahIslamicBank.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await bankTransfersOfSharjahIslamicBank.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await bankTransfersOfSharjahIslamicBank.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await bankTransfersOfSharjahIslamicBank.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await bankTransfersOfSharjahIslamicBank.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await bankTransfersOfSharjahIslamicBank.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await bankTransfersOfSharjahIslamicBank.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    bankTransfersOfSharjahIslamicBank.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await bankTransfersOfSharjahIslamicBank.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await bankTransfersOfSharjahIslamicBank.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await bankTransfersOfSharjahIslamicBank.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    bankTransfersOfSharjahIslamicBank.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    bankTransfersOfSharjahIslamicBank.selectors.submitButton as string
  );
});
