/**
 * Step Definitions for Inquire seller Refund Request Module
 *
 * This file contains all step definitions for the Inquire seller Refund Request feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module inquire-seller-refund-request.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { InquireSellerRefundRequest } from '../pages/generated/inquire-seller-refund-request.page';
 ...
 */
let page: Page;
let inquireSellerRefundRequest: InquireSellerRefundRequest;
...
function initializePageObject(): void {
  ...
  inquireSellerRefundRequest = new InquireSellerRefundRequest(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await inquireSellerRefundRequest.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await inquireSellerRefundRequest.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await inquireSellerRefundRequest.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await inquireSellerRefundRequest.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await inquireSellerRefundRequest.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await inquireSellerRefundRequest.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await inquireSellerRefundRequest.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    inquireSellerRefundRequest.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await inquireSellerRefundRequest.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await inquireSellerRefundRequest.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await inquireSellerRefundRequest.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    inquireSellerRefundRequest.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    inquireSellerRefundRequest.selectors.submitButton as string
  );
});
