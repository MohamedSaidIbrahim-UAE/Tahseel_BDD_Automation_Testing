/**
 * Step Definitions for Automatic Transaction Refund Report Module
 *
 * This file contains all step definitions for the Automatic Transaction Refund Report feature.
...
import { AutomaticTransactionRefundReport } from '../pages/generated/automatic-transaction-refund-report.page';
 ...
 */
let page: Page;
let automaticTransactionRefundReport: AutomaticTransactionRefundReport;
...
function initializePageObject(): void {
  ...
  automaticTransactionRefundReport = new AutomaticTransactionRefundReport(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await automaticTransactionRefundReport.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await automaticTransactionRefundReport.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await automaticTransactionRefundReport.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await automaticTransactionRefundReport.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await automaticTransactionRefundReport.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await automaticTransactionRefundReport.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await automaticTransactionRefundReport.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    automaticTransactionRefundReport.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await automaticTransactionRefundReport.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await automaticTransactionRefundReport.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await automaticTransactionRefundReport.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    automaticTransactionRefundReport.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    automaticTransactionRefundReport.selectors.submitButton as string
  );
});
