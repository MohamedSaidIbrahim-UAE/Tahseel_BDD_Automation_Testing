/**
 * Step Definitions for Cheque Payment Details Report Module
 *
 * This file contains all step definitions for the Cheque Payment Details Report feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module cheque-payment-details-report.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { ChequePaymentDetailsReport } from '../pages/generated/cheque-payment-details-report.page';
 ...
 */
let page: Page;
let chequePaymentDetailsReport: ChequePaymentDetailsReport;
...
function initializePageObject(): void {
  ...
  chequePaymentDetailsReport = new ChequePaymentDetailsReport(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await chequePaymentDetailsReport.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await chequePaymentDetailsReport.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await chequePaymentDetailsReport.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await chequePaymentDetailsReport.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await chequePaymentDetailsReport.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await chequePaymentDetailsReport.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await chequePaymentDetailsReport.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    chequePaymentDetailsReport.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await chequePaymentDetailsReport.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await chequePaymentDetailsReport.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await chequePaymentDetailsReport.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    chequePaymentDetailsReport.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    chequePaymentDetailsReport.selectors.submitButton as string
  );
});
