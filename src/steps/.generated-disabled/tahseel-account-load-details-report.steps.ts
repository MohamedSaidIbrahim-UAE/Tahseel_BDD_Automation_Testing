/**
 * Step Definitions for Tahseel Account Load Details Report Module
 *
 * This file contains all step definitions for the Tahseel Account Load Details Report feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module tahseel-account-load-details-report.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { TahseelAccountLoadDetailsReport } from '../pages/generated/tahseel-account-load-details-report.page';
 ...
 */
let page: Page;
let tahseelAccountLoadDetailsReport: TahseelAccountLoadDetailsReport;
...
function initializePageObject(): void {
  ...
  tahseelAccountLoadDetailsReport = new TahseelAccountLoadDetailsReport(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await tahseelAccountLoadDetailsReport.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await tahseelAccountLoadDetailsReport.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await tahseelAccountLoadDetailsReport.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await tahseelAccountLoadDetailsReport.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await tahseelAccountLoadDetailsReport.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await tahseelAccountLoadDetailsReport.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await tahseelAccountLoadDetailsReport.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    tahseelAccountLoadDetailsReport.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await tahseelAccountLoadDetailsReport.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await tahseelAccountLoadDetailsReport.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await tahseelAccountLoadDetailsReport.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    tahseelAccountLoadDetailsReport.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    tahseelAccountLoadDetailsReport.selectors.submitButton as string
  );
});
