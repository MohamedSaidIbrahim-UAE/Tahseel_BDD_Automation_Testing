/**
 * Step Definitions for Detailed Report of POS Transactions by Revenue Source Module
 *
 * This file contains all step definitions for the Detailed Report of POS Transactions by Revenue Source feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module detailed-report-of-pos-transactions-by-revenue-source.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { DetailedReportOfPosTransactionsByRevenueSource } from '../pages/generated/detailed-report-of-pos-transactions-by-revenue-source.page';
 ...
 */
let page: Page;
let detailedReportOfPosTransactionsByRevenueSource: DetailedReportOfPosTransactionsByRevenueSource;
...
function initializePageObject(): void {
  ...
  detailedReportOfPosTransactionsByRevenueSource = new DetailedReportOfPosTransactionsByRevenueSource(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await detailedReportOfPosTransactionsByRevenueSource.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await detailedReportOfPosTransactionsByRevenueSource.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await detailedReportOfPosTransactionsByRevenueSource.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await detailedReportOfPosTransactionsByRevenueSource.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await detailedReportOfPosTransactionsByRevenueSource.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await detailedReportOfPosTransactionsByRevenueSource.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await detailedReportOfPosTransactionsByRevenueSource.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    detailedReportOfPosTransactionsByRevenueSource.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await detailedReportOfPosTransactionsByRevenueSource.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await detailedReportOfPosTransactionsByRevenueSource.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await detailedReportOfPosTransactionsByRevenueSource.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    detailedReportOfPosTransactionsByRevenueSource.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    detailedReportOfPosTransactionsByRevenueSource.selectors.submitButton as string
  );
});
