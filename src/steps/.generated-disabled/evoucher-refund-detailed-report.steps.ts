/**
 * Step Definitions for Evoucher Refund Detailed Report Module
 *
 * This file contains all step definitions for the Evoucher Refund Detailed Report feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module evoucher-refund-detailed-report.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { EvoucherRefundDetailedReport } from '../pages/generated/evoucher-refund-detailed-report.page';
 ...
 */
let page: Page;
let evoucherRefundDetailedReport: EvoucherRefundDetailedReport;
...
function initializePageObject(): void {
  ...
  evoucherRefundDetailedReport = new EvoucherRefundDetailedReport(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await evoucherRefundDetailedReport.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await evoucherRefundDetailedReport.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await evoucherRefundDetailedReport.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await evoucherRefundDetailedReport.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await evoucherRefundDetailedReport.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await evoucherRefundDetailedReport.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await evoucherRefundDetailedReport.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    evoucherRefundDetailedReport.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await evoucherRefundDetailedReport.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await evoucherRefundDetailedReport.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await evoucherRefundDetailedReport.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    evoucherRefundDetailedReport.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    evoucherRefundDetailedReport.selectors.submitButton as string
  );
});
