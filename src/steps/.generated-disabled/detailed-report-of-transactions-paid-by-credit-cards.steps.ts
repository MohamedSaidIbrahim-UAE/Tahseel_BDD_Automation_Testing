/**
 * Step Definitions for Detailed Report of Transactions paid by Credit cards Module
 *
 * This file contains all step definitions for the Detailed Report of Transactions paid by Credit cards feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module detailed-report-of-transactions-paid-by-credit-cards.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { DetailedReportOfTransactionsPaidByCreditCards } from '../pages/generated/detailed-report-of-transactions-paid-by-credit-cards.page';
 ...
 */
let page: Page;
let detailedReportOfTransactionsPaidByCreditCards: DetailedReportOfTransactionsPaidByCreditCards;
...
function initializePageObject(): void {
  ...
  detailedReportOfTransactionsPaidByCreditCards = new DetailedReportOfTransactionsPaidByCreditCards(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await detailedReportOfTransactionsPaidByCreditCards.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await detailedReportOfTransactionsPaidByCreditCards.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await detailedReportOfTransactionsPaidByCreditCards.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await detailedReportOfTransactionsPaidByCreditCards.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await detailedReportOfTransactionsPaidByCreditCards.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await detailedReportOfTransactionsPaidByCreditCards.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await detailedReportOfTransactionsPaidByCreditCards.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    detailedReportOfTransactionsPaidByCreditCards.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await detailedReportOfTransactionsPaidByCreditCards.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await detailedReportOfTransactionsPaidByCreditCards.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await detailedReportOfTransactionsPaidByCreditCards.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    detailedReportOfTransactionsPaidByCreditCards.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    detailedReportOfTransactionsPaidByCreditCards.selectors.submitButton as string
  );
});
