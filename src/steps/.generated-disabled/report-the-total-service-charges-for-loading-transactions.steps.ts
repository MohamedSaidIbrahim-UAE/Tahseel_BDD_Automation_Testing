/**
 * Step Definitions for Report the total service charges for loading Transactions Module
 *
 * This file contains all step definitions for the Report the total service charges for loading Transactions feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module report-the-total-service-charges-for-loading-transactions.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { ReportTheTotalServiceChargesForLoadingTransactions } from '../pages/generated/report-the-total-service-charges-for-loading-transactions.page';
 ...
 */
let page: Page;
let reportTheTotalServiceChargesForLoadingTransactions: ReportTheTotalServiceChargesForLoadingTransactions;
...
function initializePageObject(): void {
  ...
  reportTheTotalServiceChargesForLoadingTransactions = new ReportTheTotalServiceChargesForLoadingTransactions(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await reportTheTotalServiceChargesForLoadingTransactions.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await reportTheTotalServiceChargesForLoadingTransactions.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await reportTheTotalServiceChargesForLoadingTransactions.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await reportTheTotalServiceChargesForLoadingTransactions.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await reportTheTotalServiceChargesForLoadingTransactions.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await reportTheTotalServiceChargesForLoadingTransactions.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await reportTheTotalServiceChargesForLoadingTransactions.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    reportTheTotalServiceChargesForLoadingTransactions.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await reportTheTotalServiceChargesForLoadingTransactions.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await reportTheTotalServiceChargesForLoadingTransactions.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await reportTheTotalServiceChargesForLoadingTransactions.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    reportTheTotalServiceChargesForLoadingTransactions.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    reportTheTotalServiceChargesForLoadingTransactions.selectors.submitButton as string
  );
});
