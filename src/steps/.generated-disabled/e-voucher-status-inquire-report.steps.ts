/**
 * Step Definitions for E-Voucher Status Inquire Report Module
 *
 * This file contains all step definitions for the E-Voucher Status Inquire Report feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module e-voucher-status-inquire-report.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { EVoucherStatusInquireReport } from '../pages/generated/e-voucher-status-inquire-report.page';
 ...
 */
let page: Page;
let eVoucherStatusInquireReport: EVoucherStatusInquireReport;
...
function initializePageObject(): void {
  ...
  eVoucherStatusInquireReport = new EVoucherStatusInquireReport(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await eVoucherStatusInquireReport.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await eVoucherStatusInquireReport.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await eVoucherStatusInquireReport.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await eVoucherStatusInquireReport.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await eVoucherStatusInquireReport.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await eVoucherStatusInquireReport.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await eVoucherStatusInquireReport.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    eVoucherStatusInquireReport.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await eVoucherStatusInquireReport.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await eVoucherStatusInquireReport.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await eVoucherStatusInquireReport.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    eVoucherStatusInquireReport.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    eVoucherStatusInquireReport.selectors.submitButton as string
  );
});
