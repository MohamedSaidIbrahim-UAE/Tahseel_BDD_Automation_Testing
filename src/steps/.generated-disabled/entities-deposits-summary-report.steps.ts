/**
 * Step Definitions for Entities Deposits - Summary Report Module
 *
 * This file contains all step definitions for the Entities Deposits - Summary Report feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module entities-deposits-summary-report.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { EntitiesDepositsSummaryReport } from '../pages/generated/entities-deposits-summary-report.page';
 ...
 */
let page: Page;
let entitiesDepositsSummaryReport: EntitiesDepositsSummaryReport;
...
function initializePageObject(): void {
  ...
  entitiesDepositsSummaryReport = new EntitiesDepositsSummaryReport(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await entitiesDepositsSummaryReport.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await entitiesDepositsSummaryReport.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await entitiesDepositsSummaryReport.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await entitiesDepositsSummaryReport.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await entitiesDepositsSummaryReport.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await entitiesDepositsSummaryReport.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await entitiesDepositsSummaryReport.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    entitiesDepositsSummaryReport.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await entitiesDepositsSummaryReport.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await entitiesDepositsSummaryReport.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await entitiesDepositsSummaryReport.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    entitiesDepositsSummaryReport.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    entitiesDepositsSummaryReport.selectors.submitButton as string
  );
});
