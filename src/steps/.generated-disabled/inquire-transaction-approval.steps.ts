/**
 * Step Definitions for Inquire Transaction Approval Module
 *
 * This file contains all step definitions for the Inquire Transaction Approval feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module inquire-transaction-approval.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { InquireTransactionApproval } from '../pages/generated/inquire-transaction-approval.page';
 ...
 */
let page: Page;
let inquireTransactionApproval: InquireTransactionApproval;
...
function initializePageObject(): void {
  ...
  inquireTransactionApproval = new InquireTransactionApproval(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await inquireTransactionApproval.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await inquireTransactionApproval.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await inquireTransactionApproval.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await inquireTransactionApproval.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await inquireTransactionApproval.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await inquireTransactionApproval.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await inquireTransactionApproval.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    inquireTransactionApproval.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await inquireTransactionApproval.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await inquireTransactionApproval.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await inquireTransactionApproval.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    inquireTransactionApproval.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    inquireTransactionApproval.selectors.submitButton as string
  );
});
