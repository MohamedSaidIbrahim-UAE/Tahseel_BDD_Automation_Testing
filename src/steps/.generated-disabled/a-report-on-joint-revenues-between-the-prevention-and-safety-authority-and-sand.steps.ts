/**
 * Step Definitions for A report on joint revenues between the Prevention and Safety Authority and SAND Module
 *
 * This file contains all step definitions for the A report on joint revenues between the Prevention and Safety Authority and SAND feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module a-report-on-joint-revenues-between-the-prevention-and-safety-authority-and-sand.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { AReportOnJointRevenuesBetweenThePreventionAndSafetyAuthorityAndSand } from '../pages/generated/a-report-on-joint-revenues-between-the-prevention-and-safety-authority-and-sand.page';
 ...
 */
let page: Page;
let aReportOnJointRevenuesBetweenThePreventionAndSafetyAuthorityAndSand: AReportOnJointRevenuesBetweenThePreventionAndSafetyAuthorityAndSand;
...
function initializePageObject(): void {
  ...
  aReportOnJointRevenuesBetweenThePreventionAndSafetyAuthorityAndSand = new AReportOnJointRevenuesBetweenThePreventionAndSafetyAuthorityAndSand(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await aReportOnJointRevenuesBetweenThePreventionAndSafetyAuthorityAndSand.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await aReportOnJointRevenuesBetweenThePreventionAndSafetyAuthorityAndSand.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await aReportOnJointRevenuesBetweenThePreventionAndSafetyAuthorityAndSand.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await aReportOnJointRevenuesBetweenThePreventionAndSafetyAuthorityAndSand.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await aReportOnJointRevenuesBetweenThePreventionAndSafetyAuthorityAndSand.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await aReportOnJointRevenuesBetweenThePreventionAndSafetyAuthorityAndSand.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await aReportOnJointRevenuesBetweenThePreventionAndSafetyAuthorityAndSand.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    aReportOnJointRevenuesBetweenThePreventionAndSafetyAuthorityAndSand.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await aReportOnJointRevenuesBetweenThePreventionAndSafetyAuthorityAndSand.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await aReportOnJointRevenuesBetweenThePreventionAndSafetyAuthorityAndSand.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await aReportOnJointRevenuesBetweenThePreventionAndSafetyAuthorityAndSand.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    aReportOnJointRevenuesBetweenThePreventionAndSafetyAuthorityAndSand.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    aReportOnJointRevenuesBetweenThePreventionAndSafetyAuthorityAndSand.selectors.submitButton as string
  );
});
