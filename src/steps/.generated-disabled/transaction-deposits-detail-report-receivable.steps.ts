/**
 * Step Definitions for Transaction deposits detail Report (receivable) Module
 *
 * This file contains all step definitions for the Transaction deposits detail Report (receivable) feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module transaction-deposits-detail-report-receivable.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { TransactionDepositsDetailReportreceivable } from '../pages/generated/transaction-deposits-detail-report-receivable.page';
 ...
 */
let page: Page;
let transactionDepositsDetailReportreceivable: TransactionDepositsDetailReportreceivable;
...
function initializePageObject(): void {
  ...
  transactionDepositsDetailReportreceivable = new TransactionDepositsDetailReportreceivable(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await transactionDepositsDetailReportreceivable.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await transactionDepositsDetailReportreceivable.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await transactionDepositsDetailReportreceivable.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await transactionDepositsDetailReportreceivable.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await transactionDepositsDetailReportreceivable.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await transactionDepositsDetailReportreceivable.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await transactionDepositsDetailReportreceivable.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    transactionDepositsDetailReportreceivable.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await transactionDepositsDetailReportreceivable.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await transactionDepositsDetailReportreceivable.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await transactionDepositsDetailReportreceivable.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    transactionDepositsDetailReportreceivable.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    transactionDepositsDetailReportreceivable.selectors.submitButton as string
  );
});
