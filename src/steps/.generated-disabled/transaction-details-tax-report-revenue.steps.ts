/**
 * Step Definitions for Transaction Details Tax Report - Revenue Module
 *
 * This file contains all step definitions for the Transaction Details Tax Report - Revenue feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module transaction-details-tax-report-revenue.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { TransactionDetailsTaxReportRevenue } from '../pages/generated/transaction-details-tax-report-revenue.page';
 ...
 */
let page: Page;
let transactionDetailsTaxReportRevenue: TransactionDetailsTaxReportRevenue;
...
function initializePageObject(): void {
  ...
  transactionDetailsTaxReportRevenue = new TransactionDetailsTaxReportRevenue(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await transactionDetailsTaxReportRevenue.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await transactionDetailsTaxReportRevenue.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await transactionDetailsTaxReportRevenue.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await transactionDetailsTaxReportRevenue.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await transactionDetailsTaxReportRevenue.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await transactionDetailsTaxReportRevenue.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await transactionDetailsTaxReportRevenue.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    transactionDetailsTaxReportRevenue.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await transactionDetailsTaxReportRevenue.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await transactionDetailsTaxReportRevenue.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await transactionDetailsTaxReportRevenue.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    transactionDetailsTaxReportRevenue.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    transactionDetailsTaxReportRevenue.selectors.submitButton as string
  );
});
