/**
 * Step Definitions for Dependant services revenue detailed reports Module
 *
 * This file contains all step definitions for the Dependant services revenue detailed reports feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module dependant-services-revenue-detailed-reports.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { DependantServicesRevenueDetailedReports } from '../pages/generated/dependant-services-revenue-detailed-reports.page';
 ...
 */
let page: Page;
let dependantServicesRevenueDetailedReports: DependantServicesRevenueDetailedReports;
...
function initializePageObject(): void {
  ...
  dependantServicesRevenueDetailedReports = new DependantServicesRevenueDetailedReports(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await dependantServicesRevenueDetailedReports.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await dependantServicesRevenueDetailedReports.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await dependantServicesRevenueDetailedReports.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await dependantServicesRevenueDetailedReports.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await dependantServicesRevenueDetailedReports.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await dependantServicesRevenueDetailedReports.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await dependantServicesRevenueDetailedReports.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    dependantServicesRevenueDetailedReports.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await dependantServicesRevenueDetailedReports.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await dependantServicesRevenueDetailedReports.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await dependantServicesRevenueDetailedReports.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    dependantServicesRevenueDetailedReports.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    dependantServicesRevenueDetailedReports.selectors.submitButton as string
  );
});
