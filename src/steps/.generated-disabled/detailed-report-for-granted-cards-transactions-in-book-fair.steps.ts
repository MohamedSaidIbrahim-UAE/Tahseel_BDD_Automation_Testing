/**
 * Step Definitions for Detailed Report For Granted Cards Transactions in Book Fair Module
 *
 * This file contains all step definitions for the Detailed Report For Granted Cards Transactions in Book Fair feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module detailed-report-for-granted-cards-transactions-in-book-fair.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { DetailedReportForGrantedCardsTransactionsInBookFair } from '../pages/generated/detailed-report-for-granted-cards-transactions-in-book-fair.page';
 ...
 */
let page: Page;
let detailedReportForGrantedCardsTransactionsInBookFair: DetailedReportForGrantedCardsTransactionsInBookFair;
...
function initializePageObject(): void {
  ...
  detailedReportForGrantedCardsTransactionsInBookFair = new DetailedReportForGrantedCardsTransactionsInBookFair(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await detailedReportForGrantedCardsTransactionsInBookFair.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await detailedReportForGrantedCardsTransactionsInBookFair.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await detailedReportForGrantedCardsTransactionsInBookFair.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await detailedReportForGrantedCardsTransactionsInBookFair.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await detailedReportForGrantedCardsTransactionsInBookFair.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await detailedReportForGrantedCardsTransactionsInBookFair.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await detailedReportForGrantedCardsTransactionsInBookFair.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    detailedReportForGrantedCardsTransactionsInBookFair.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await detailedReportForGrantedCardsTransactionsInBookFair.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await detailedReportForGrantedCardsTransactionsInBookFair.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await detailedReportForGrantedCardsTransactionsInBookFair.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    detailedReportForGrantedCardsTransactionsInBookFair.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    detailedReportForGrantedCardsTransactionsInBookFair.selectors.submitButton as string
  );
});
