/**
 * Step Definitions for Deposit Refund Requests Inquiry Module
 *
 * This file contains all step definitions for the Deposit Refund Requests Inquiry feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module deposit-refund-requests-inquiry.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { DepositRefundRequestsInquiry } from '../pages/generated/deposit-refund-requests-inquiry.page';
 ...
 */
let page: Page;
let depositRefundRequestsInquiry: DepositRefundRequestsInquiry;
...
function initializePageObject(): void {
  ...
  depositRefundRequestsInquiry = new DepositRefundRequestsInquiry(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await depositRefundRequestsInquiry.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await depositRefundRequestsInquiry.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await depositRefundRequestsInquiry.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await depositRefundRequestsInquiry.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await depositRefundRequestsInquiry.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await depositRefundRequestsInquiry.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await depositRefundRequestsInquiry.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    depositRefundRequestsInquiry.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await depositRefundRequestsInquiry.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await depositRefundRequestsInquiry.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await depositRefundRequestsInquiry.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    depositRefundRequestsInquiry.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    depositRefundRequestsInquiry.selectors.submitButton as string
  );
});
