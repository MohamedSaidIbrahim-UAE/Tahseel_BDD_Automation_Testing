/**
 * Step Definitions for Detailed Tahseel Card Report Module
 *
 * This file contains all step definitions for the Detailed Tahseel Card Report feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module detailed-tahseel-card-report.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { DetailedTahseelCardReport } from '../pages/generated/detailed-tahseel-card-report.page';
 ...
 */
let page: Page;
let detailedTahseelCardReport: DetailedTahseelCardReport;
...
function initializePageObject(): void {
  ...
  detailedTahseelCardReport = new DetailedTahseelCardReport(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await detailedTahseelCardReport.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await detailedTahseelCardReport.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await detailedTahseelCardReport.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await detailedTahseelCardReport.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await detailedTahseelCardReport.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await detailedTahseelCardReport.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await detailedTahseelCardReport.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    detailedTahseelCardReport.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await detailedTahseelCardReport.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await detailedTahseelCardReport.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await detailedTahseelCardReport.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    detailedTahseelCardReport.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    detailedTahseelCardReport.selectors.submitButton as string
  );
});
