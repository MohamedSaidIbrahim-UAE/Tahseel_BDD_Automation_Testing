/**
 * Step Definitions for Inquire Deduct Transaction Module
 *
 * This file contains all step definitions for the Inquire Deduct Transaction feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module inquire-deduct-transaction.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { InquireDeductTransaction } from '../pages/generated/inquire-deduct-transaction.page';
 ...
 */
let page: Page;
let inquireDeductTransaction: InquireDeductTransaction;
...
function initializePageObject(): void {
  ...
  inquireDeductTransaction = new InquireDeductTransaction(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await inquireDeductTransaction.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await inquireDeductTransaction.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await inquireDeductTransaction.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await inquireDeductTransaction.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await inquireDeductTransaction.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await inquireDeductTransaction.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await inquireDeductTransaction.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    inquireDeductTransaction.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await inquireDeductTransaction.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await inquireDeductTransaction.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await inquireDeductTransaction.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    inquireDeductTransaction.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    inquireDeductTransaction.selectors.submitButton as string
  );
});
