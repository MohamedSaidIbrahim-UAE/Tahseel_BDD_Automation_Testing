/**
 * Step Definitions for Tahseel Account Details Module
 *
 * This file contains all step definitions for the Tahseel Account Details feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module tahseel-account-details.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { TahseelAccountDetails } from '../pages/generated/tahseel-account-details.page';
 ...
 */
let page: Page;
let tahseelAccountDetails: TahseelAccountDetails;
...
function initializePageObject(): void {
  ...
  tahseelAccountDetails = new TahseelAccountDetails(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await tahseelAccountDetails.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await tahseelAccountDetails.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await tahseelAccountDetails.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await tahseelAccountDetails.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await tahseelAccountDetails.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await tahseelAccountDetails.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await tahseelAccountDetails.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    tahseelAccountDetails.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await tahseelAccountDetails.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await tahseelAccountDetails.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await tahseelAccountDetails.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    tahseelAccountDetails.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    tahseelAccountDetails.selectors.submitButton as string
  );
});
