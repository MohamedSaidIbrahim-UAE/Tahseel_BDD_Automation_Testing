/**
 * Step Definitions for Adjustment Registration Request Module
 *
 * This file contains all step definitions for the Adjustment Registration Request feature.
...
import { AdjustmentRegistrationRequest } from '../pages/generated/adjustment-registration-request.page';
 ...
 */
let page: Page;
let adjustmentRegistrationRequest: AdjustmentRegistrationRequest;
...
function initializePageObject(): void {
  ...
  adjustmentRegistrationRequest = new AdjustmentRegistrationRequest(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await adjustmentRegistrationRequest.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await adjustmentRegistrationRequest.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await adjustmentRegistrationRequest.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await adjustmentRegistrationRequest.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await adjustmentRegistrationRequest.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await adjustmentRegistrationRequest.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await adjustmentRegistrationRequest.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    adjustmentRegistrationRequest.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await adjustmentRegistrationRequest.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await adjustmentRegistrationRequest.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await adjustmentRegistrationRequest.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    adjustmentRegistrationRequest.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    adjustmentRegistrationRequest.selectors.submitButton as string
  );
});
