/**
 * Step Definitions for Adjustment Registration Requests Module
 *
 * This file contains all step definitions for the Adjustment Registration Requests feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module adjustment-registration-requests.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { AdjustmentRegistrationRequests } from '../pages/generated/adjustment-registration-requests.page';
 ...
 */
let page: Page;
let adjustmentRegistrationRequests: AdjustmentRegistrationRequests;
...
function initializePageObject(): void {
  ...
  adjustmentRegistrationRequests = new AdjustmentRegistrationRequests(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await adjustmentRegistrationRequests.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await adjustmentRegistrationRequests.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await adjustmentRegistrationRequests.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await adjustmentRegistrationRequests.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await adjustmentRegistrationRequests.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await adjustmentRegistrationRequests.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await adjustmentRegistrationRequests.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    adjustmentRegistrationRequests.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await adjustmentRegistrationRequests.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await adjustmentRegistrationRequests.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await adjustmentRegistrationRequests.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    adjustmentRegistrationRequests.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    adjustmentRegistrationRequests.selectors.submitButton as string
  );
});
