/**
 * Step Definitions for Corporate Card Balance Report Module
 *
 * This file contains all step definitions for the Corporate Card Balance Report feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module corporate-card-balance-report.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { CorporateCardBalanceReport } from '../pages/generated/corporate-card-balance-report.page';
 ...
 */
let page: Page;
let corporateCardBalanceReport: CorporateCardBalanceReport;
...
function initializePageObject(): void {
  ...
  corporateCardBalanceReport = new CorporateCardBalanceReport(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await corporateCardBalanceReport.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await corporateCardBalanceReport.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await corporateCardBalanceReport.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await corporateCardBalanceReport.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await corporateCardBalanceReport.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await corporateCardBalanceReport.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await corporateCardBalanceReport.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    corporateCardBalanceReport.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await corporateCardBalanceReport.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await corporateCardBalanceReport.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await corporateCardBalanceReport.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    corporateCardBalanceReport.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    corporateCardBalanceReport.selectors.submitButton as string
  );
});
