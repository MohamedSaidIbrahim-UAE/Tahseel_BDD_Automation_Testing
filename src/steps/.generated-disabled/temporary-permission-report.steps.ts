/**
 * Step Definitions for Temporary permission report Module
 *
 * This file contains all step definitions for the Temporary permission report feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module temporary-permission-report.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { TemporaryPermissionReport } from '../pages/generated/temporary-permission-report.page';
 ...
 */
let page: Page;
let temporaryPermissionReport: TemporaryPermissionReport;
...
function initializePageObject(): void {
  ...
  temporaryPermissionReport = new TemporaryPermissionReport(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await temporaryPermissionReport.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await temporaryPermissionReport.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await temporaryPermissionReport.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await temporaryPermissionReport.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await temporaryPermissionReport.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await temporaryPermissionReport.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await temporaryPermissionReport.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    temporaryPermissionReport.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await temporaryPermissionReport.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await temporaryPermissionReport.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await temporaryPermissionReport.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    temporaryPermissionReport.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    temporaryPermissionReport.selectors.submitButton as string
  );
});
