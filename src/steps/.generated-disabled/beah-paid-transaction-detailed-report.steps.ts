/**
 * Step Definitions for Beah paid transaction detailed report Module
 *
 * This file contains all step definitions for the Beah paid transaction detailed report feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module beah-paid-transaction-detailed-report.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { BeahPaidTransactionDetailedReport } from '../pages/generated/beah-paid-transaction-detailed-report.page';
 ...
 */
let page: Page;
let beahPaidTransactionDetailedReport: BeahPaidTransactionDetailedReport;
...
function initializePageObject(): void {
  ...
  beahPaidTransactionDetailedReport = new BeahPaidTransactionDetailedReport(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await beahPaidTransactionDetailedReport.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await beahPaidTransactionDetailedReport.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await beahPaidTransactionDetailedReport.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await beahPaidTransactionDetailedReport.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await beahPaidTransactionDetailedReport.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await beahPaidTransactionDetailedReport.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await beahPaidTransactionDetailedReport.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    beahPaidTransactionDetailedReport.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await beahPaidTransactionDetailedReport.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await beahPaidTransactionDetailedReport.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await beahPaidTransactionDetailedReport.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    beahPaidTransactionDetailedReport.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    beahPaidTransactionDetailedReport.selectors.submitButton as string
  );
});
