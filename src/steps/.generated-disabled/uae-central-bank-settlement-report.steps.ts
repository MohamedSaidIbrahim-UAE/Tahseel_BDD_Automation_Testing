/**
 * Step Definitions for UAE central bank settlement report Module
 *
 * This file contains all step definitions for the UAE central bank settlement report feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module uae-central-bank-settlement-report.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { UaeCentralBankSettlementReport } from '../pages/generated/uae-central-bank-settlement-report.page';
 ...
 */
let page: Page;
let uaeCentralBankSettlementReport: UaeCentralBankSettlementReport;
...
function initializePageObject(): void {
  ...
  uaeCentralBankSettlementReport = new UaeCentralBankSettlementReport(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await uaeCentralBankSettlementReport.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await uaeCentralBankSettlementReport.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await uaeCentralBankSettlementReport.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await uaeCentralBankSettlementReport.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await uaeCentralBankSettlementReport.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await uaeCentralBankSettlementReport.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await uaeCentralBankSettlementReport.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    uaeCentralBankSettlementReport.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await uaeCentralBankSettlementReport.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await uaeCentralBankSettlementReport.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await uaeCentralBankSettlementReport.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    uaeCentralBankSettlementReport.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    uaeCentralBankSettlementReport.selectors.submitButton as string
  );
});
