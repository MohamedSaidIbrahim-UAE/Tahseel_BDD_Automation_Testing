/**
 * Step Definitions for Permanent permission Log report Module
 *
 * This file contains all step definitions for the Permanent permission Log report feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module permanent-permission-log-report.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { PermanentPermissionLogReport } from '../pages/generated/permanent-permission-log-report.page';
 ...
 */
let page: Page;
let permanentPermissionLogReport: PermanentPermissionLogReport;
...
function initializePageObject(): void {
  ...
  permanentPermissionLogReport = new PermanentPermissionLogReport(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await permanentPermissionLogReport.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await permanentPermissionLogReport.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await permanentPermissionLogReport.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await permanentPermissionLogReport.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await permanentPermissionLogReport.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await permanentPermissionLogReport.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await permanentPermissionLogReport.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    permanentPermissionLogReport.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await permanentPermissionLogReport.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await permanentPermissionLogReport.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await permanentPermissionLogReport.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    permanentPermissionLogReport.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    permanentPermissionLogReport.selectors.submitButton as string
  );
});
