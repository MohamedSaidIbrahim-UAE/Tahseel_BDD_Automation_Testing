/**
 * Step Definitions for Aggregated Transactions Report paid by Credit cards Module
 *
 * This file contains all step definitions for the Aggregated Transactions Report paid by Credit cards feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module aggregated-transactions-report-paid-by-credit-cards.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { AggregatedTransactionsReportPaidByCreditCards } from '../pages/generated/aggregated-transactions-report-paid-by-credit-cards.page';
 ...
 */
let page: Page;
let aggregatedTransactionsReportPaidByCreditCards: AggregatedTransactionsReportPaidByCreditCards;
...
function initializePageObject(): void {
  ...
  aggregatedTransactionsReportPaidByCreditCards = new AggregatedTransactionsReportPaidByCreditCards(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await aggregatedTransactionsReportPaidByCreditCards.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await aggregatedTransactionsReportPaidByCreditCards.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await aggregatedTransactionsReportPaidByCreditCards.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await aggregatedTransactionsReportPaidByCreditCards.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await aggregatedTransactionsReportPaidByCreditCards.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await aggregatedTransactionsReportPaidByCreditCards.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await aggregatedTransactionsReportPaidByCreditCards.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    aggregatedTransactionsReportPaidByCreditCards.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await aggregatedTransactionsReportPaidByCreditCards.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await aggregatedTransactionsReportPaidByCreditCards.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await aggregatedTransactionsReportPaidByCreditCards.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    aggregatedTransactionsReportPaidByCreditCards.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    aggregatedTransactionsReportPaidByCreditCards.selectors.submitButton as string
  );
});
