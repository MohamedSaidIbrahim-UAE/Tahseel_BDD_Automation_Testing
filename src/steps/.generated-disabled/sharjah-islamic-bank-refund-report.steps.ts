/**
 * Step Definitions for Sharjah islamic bank refund report Module
 *
 * This file contains all step definitions for the Sharjah islamic bank refund report feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module sharjah-islamic-bank-refund-report.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { SharjahIslamicBankRefundReport } from '../pages/generated/sharjah-islamic-bank-refund-report.page';
 ...
 */
let page: Page;
let sharjahIslamicBankRefundReport: SharjahIslamicBankRefundReport;
...
function initializePageObject(): void {
  ...
  sharjahIslamicBankRefundReport = new SharjahIslamicBankRefundReport(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await sharjahIslamicBankRefundReport.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await sharjahIslamicBankRefundReport.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await sharjahIslamicBankRefundReport.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await sharjahIslamicBankRefundReport.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await sharjahIslamicBankRefundReport.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await sharjahIslamicBankRefundReport.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await sharjahIslamicBankRefundReport.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    sharjahIslamicBankRefundReport.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await sharjahIslamicBankRefundReport.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await sharjahIslamicBankRefundReport.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await sharjahIslamicBankRefundReport.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    sharjahIslamicBankRefundReport.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    sharjahIslamicBankRefundReport.selectors.submitButton as string
  );
});
