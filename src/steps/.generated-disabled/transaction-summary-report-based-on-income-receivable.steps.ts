/**
 * Step Definitions for Transaction Summary Report Based on Income (receivable) Module
 *
 * This file contains all step definitions for the Transaction Summary Report Based on Income (receivable) feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module transaction-summary-report-based-on-income-receivable.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { TransactionSummaryReportBasedOnIncomereceivable } from '../pages/generated/transaction-summary-report-based-on-income-receivable.page';
 ...
 */
let page: Page;
let transactionSummaryReportBasedOnIncomereceivable: TransactionSummaryReportBasedOnIncomereceivable;
...
function initializePageObject(): void {
  ...
  transactionSummaryReportBasedOnIncomereceivable = new TransactionSummaryReportBasedOnIncomereceivable(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await transactionSummaryReportBasedOnIncomereceivable.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await transactionSummaryReportBasedOnIncomereceivable.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await transactionSummaryReportBasedOnIncomereceivable.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await transactionSummaryReportBasedOnIncomereceivable.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await transactionSummaryReportBasedOnIncomereceivable.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await transactionSummaryReportBasedOnIncomereceivable.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await transactionSummaryReportBasedOnIncomereceivable.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    transactionSummaryReportBasedOnIncomereceivable.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await transactionSummaryReportBasedOnIncomereceivable.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await transactionSummaryReportBasedOnIncomereceivable.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await transactionSummaryReportBasedOnIncomereceivable.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    transactionSummaryReportBasedOnIncomereceivable.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    transactionSummaryReportBasedOnIncomereceivable.selectors.submitButton as string
  );
});
