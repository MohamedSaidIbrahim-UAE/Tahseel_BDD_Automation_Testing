/**
 * Step Definitions for Payment Card Statistic Report Module
 *
 * This file contains all step definitions for the Payment Card Statistic Report feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module payment-card-statistic-report.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { PaymentCardStatisticReport } from '../pages/generated/payment-card-statistic-report.page';
 ...
 */
let page: Page;
let paymentCardStatisticReport: PaymentCardStatisticReport;
...
function initializePageObject(): void {
  ...
  paymentCardStatisticReport = new PaymentCardStatisticReport(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await paymentCardStatisticReport.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await paymentCardStatisticReport.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await paymentCardStatisticReport.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await paymentCardStatisticReport.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await paymentCardStatisticReport.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await paymentCardStatisticReport.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await paymentCardStatisticReport.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    paymentCardStatisticReport.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await paymentCardStatisticReport.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await paymentCardStatisticReport.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await paymentCardStatisticReport.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    paymentCardStatisticReport.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    paymentCardStatisticReport.selectors.submitButton as string
  );
});
