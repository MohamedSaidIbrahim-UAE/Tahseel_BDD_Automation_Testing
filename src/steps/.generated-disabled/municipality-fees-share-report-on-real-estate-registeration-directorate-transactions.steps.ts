/**
 * Step Definitions for Municipality Fees Share Report on Real Estate Registeration Directorate Transactions Module
 *
 * This file contains all step definitions for the Municipality Fees Share Report on Real Estate Registeration Directorate Transactions feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module municipality-fees-share-report-on-real-estate-registeration-directorate-transactions.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { MunicipalityFeesShareReportOnRealEstateRegisterationDirectorateTransactions } from '../pages/generated/municipality-fees-share-report-on-real-estate-registeration-directorate-transactions.page';
 ...
 */
let page: Page;
let municipalityFeesShareReportOnRealEstateRegisterationDirectorateTransactions: MunicipalityFeesShareReportOnRealEstateRegisterationDirectorateTransactions;
...
function initializePageObject(): void {
  ...
  municipalityFeesShareReportOnRealEstateRegisterationDirectorateTransactions = new MunicipalityFeesShareReportOnRealEstateRegisterationDirectorateTransactions(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await municipalityFeesShareReportOnRealEstateRegisterationDirectorateTransactions.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await municipalityFeesShareReportOnRealEstateRegisterationDirectorateTransactions.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await municipalityFeesShareReportOnRealEstateRegisterationDirectorateTransactions.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await municipalityFeesShareReportOnRealEstateRegisterationDirectorateTransactions.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await municipalityFeesShareReportOnRealEstateRegisterationDirectorateTransactions.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await municipalityFeesShareReportOnRealEstateRegisterationDirectorateTransactions.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await municipalityFeesShareReportOnRealEstateRegisterationDirectorateTransactions.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    municipalityFeesShareReportOnRealEstateRegisterationDirectorateTransactions.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await municipalityFeesShareReportOnRealEstateRegisterationDirectorateTransactions.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await municipalityFeesShareReportOnRealEstateRegisterationDirectorateTransactions.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await municipalityFeesShareReportOnRealEstateRegisterationDirectorateTransactions.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    municipalityFeesShareReportOnRealEstateRegisterationDirectorateTransactions.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    municipalityFeesShareReportOnRealEstateRegisterationDirectorateTransactions.selectors.submitButton as string
  );
});
