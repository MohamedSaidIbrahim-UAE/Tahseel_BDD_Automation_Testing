/**
 * Step Definitions for Pre-Authorized Transaction Workflow Module
 *
 * This file contains all step definitions for the Pre-Authorized Transaction Workflow feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module pre-authorized-transaction-workflow.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { PreAuthorizedTransactionWorkflow } from '../pages/generated/pre-authorized-transaction-workflow.page';
 ...
 */
let page: Page;
let preAuthorizedTransactionWorkflow: PreAuthorizedTransactionWorkflow;
...
function initializePageObject(): void {
  ...
  preAuthorizedTransactionWorkflow = new PreAuthorizedTransactionWorkflow(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await preAuthorizedTransactionWorkflow.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await preAuthorizedTransactionWorkflow.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await preAuthorizedTransactionWorkflow.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await preAuthorizedTransactionWorkflow.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await preAuthorizedTransactionWorkflow.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await preAuthorizedTransactionWorkflow.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await preAuthorizedTransactionWorkflow.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    preAuthorizedTransactionWorkflow.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await preAuthorizedTransactionWorkflow.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await preAuthorizedTransactionWorkflow.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await preAuthorizedTransactionWorkflow.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    preAuthorizedTransactionWorkflow.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    preAuthorizedTransactionWorkflow.selectors.submitButton as string
  );
});
