/**
 * Step Definitions for Pending Deposit Retain Requests Module
 *
 * This file contains all step definitions for the Pending Deposit Retain Requests feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module pending-deposit-retain-requests.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { PendingDepositRetainRequests } from '../pages/generated/pending-deposit-retain-requests.page';
 ...
 */
let page: Page;
let pendingDepositRetainRequests: PendingDepositRetainRequests;
...
function initializePageObject(): void {
  ...
  pendingDepositRetainRequests = new PendingDepositRetainRequests(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await pendingDepositRetainRequests.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await pendingDepositRetainRequests.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await pendingDepositRetainRequests.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await pendingDepositRetainRequests.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await pendingDepositRetainRequests.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await pendingDepositRetainRequests.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await pendingDepositRetainRequests.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    pendingDepositRetainRequests.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await pendingDepositRetainRequests.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await pendingDepositRetainRequests.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await pendingDepositRetainRequests.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    pendingDepositRetainRequests.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    pendingDepositRetainRequests.selectors.submitButton as string
  );
});
