/**
 * Step Definitions for Transaction Fees For All Payment Methods Module
 *
 * This file contains all step definitions for the Transaction Fees For All Payment Methods feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module transaction-fees-for-all-payment-methods.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { TransactionFeesForAllPaymentMethods } from '../pages/generated/transaction-fees-for-all-payment-methods.page';
 ...
 */
let page: Page;
let transactionFeesForAllPaymentMethods: TransactionFeesForAllPaymentMethods;
...
function initializePageObject(): void {
  ...
  transactionFeesForAllPaymentMethods = new TransactionFeesForAllPaymentMethods(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await transactionFeesForAllPaymentMethods.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await transactionFeesForAllPaymentMethods.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await transactionFeesForAllPaymentMethods.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await transactionFeesForAllPaymentMethods.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await transactionFeesForAllPaymentMethods.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await transactionFeesForAllPaymentMethods.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await transactionFeesForAllPaymentMethods.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    transactionFeesForAllPaymentMethods.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await transactionFeesForAllPaymentMethods.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await transactionFeesForAllPaymentMethods.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await transactionFeesForAllPaymentMethods.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    transactionFeesForAllPaymentMethods.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    transactionFeesForAllPaymentMethods.selectors.submitButton as string
  );
});
