/**
 * Step Definitions for Manage Adjustment Registeration Workflow Module
 *
 * This file contains all step definitions for the Manage Adjustment Registeration Workflow feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module manage-adjustment-registeration-workflow.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { ManageAdjustmentRegisterationWorkflow } from '../pages/generated/manage-adjustment-registeration-workflow.page';
 ...
 */
let page: Page;
let manageAdjustmentRegisterationWorkflow: ManageAdjustmentRegisterationWorkflow;
...
function initializePageObject(): void {
  ...
  manageAdjustmentRegisterationWorkflow = new ManageAdjustmentRegisterationWorkflow(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await manageAdjustmentRegisterationWorkflow.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await manageAdjustmentRegisterationWorkflow.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await manageAdjustmentRegisterationWorkflow.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await manageAdjustmentRegisterationWorkflow.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await manageAdjustmentRegisterationWorkflow.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await manageAdjustmentRegisterationWorkflow.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await manageAdjustmentRegisterationWorkflow.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    manageAdjustmentRegisterationWorkflow.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await manageAdjustmentRegisterationWorkflow.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await manageAdjustmentRegisterationWorkflow.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await manageAdjustmentRegisterationWorkflow.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    manageAdjustmentRegisterationWorkflow.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    manageAdjustmentRegisterationWorkflow.selectors.submitButton as string
  );
});
