/**
 * Step Definitions for Transactions Sent By Payment Link Module
 *
 * This file contains all step definitions for the Transactions Sent By Payment Link feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module transactions-sent-by-payment-link.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { TransactionsSentByPaymentLink } from '../pages/generated/transactions-sent-by-payment-link.page';
 ...
 */
let page: Page;
let transactionsSentByPaymentLink: TransactionsSentByPaymentLink;
...
function initializePageObject(): void {
  ...
  transactionsSentByPaymentLink = new TransactionsSentByPaymentLink(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await transactionsSentByPaymentLink.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await transactionsSentByPaymentLink.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await transactionsSentByPaymentLink.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await transactionsSentByPaymentLink.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await transactionsSentByPaymentLink.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await transactionsSentByPaymentLink.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await transactionsSentByPaymentLink.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    transactionsSentByPaymentLink.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await transactionsSentByPaymentLink.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await transactionsSentByPaymentLink.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await transactionsSentByPaymentLink.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    transactionsSentByPaymentLink.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    transactionsSentByPaymentLink.selectors.submitButton as string
  );
});
