/**
 * Step Definitions for Sharjah islamic bank settlement report in period Module
 *
 * This file contains all step definitions for the Sharjah islamic bank settlement report in period feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module sharjah-islamic-bank-settlement-report-in-period.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { SharjahIslamicBankSettlementReportInPeriod } from '../pages/generated/sharjah-islamic-bank-settlement-report-in-period.page';
 ...
 */
let page: Page;
let sharjahIslamicBankSettlementReportInPeriod: SharjahIslamicBankSettlementReportInPeriod;
...
function initializePageObject(): void {
  ...
  sharjahIslamicBankSettlementReportInPeriod = new SharjahIslamicBankSettlementReportInPeriod(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await sharjahIslamicBankSettlementReportInPeriod.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await sharjahIslamicBankSettlementReportInPeriod.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await sharjahIslamicBankSettlementReportInPeriod.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await sharjahIslamicBankSettlementReportInPeriod.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await sharjahIslamicBankSettlementReportInPeriod.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await sharjahIslamicBankSettlementReportInPeriod.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await sharjahIslamicBankSettlementReportInPeriod.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    sharjahIslamicBankSettlementReportInPeriod.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await sharjahIslamicBankSettlementReportInPeriod.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await sharjahIslamicBankSettlementReportInPeriod.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await sharjahIslamicBankSettlementReportInPeriod.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    sharjahIslamicBankSettlementReportInPeriod.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    sharjahIslamicBankSettlementReportInPeriod.selectors.submitButton as string
  );
});
