/**
 * Step Definitions for Detailed report of canceled transactions by revenue Module
 *
 * This file contains all step definitions for the Detailed report of canceled transactions by revenue feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module detailed-report-of-canceled-transactions-by-revenue.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { DetailedReportOfCanceledTransactionsByRevenue } from '../pages/generated/detailed-report-of-canceled-transactions-by-revenue.page';
 ...
 */
let page: Page;
let detailedReportOfCanceledTransactionsByRevenue: DetailedReportOfCanceledTransactionsByRevenue;
...
function initializePageObject(): void {
  ...
  detailedReportOfCanceledTransactionsByRevenue = new DetailedReportOfCanceledTransactionsByRevenue(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await detailedReportOfCanceledTransactionsByRevenue.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await detailedReportOfCanceledTransactionsByRevenue.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await detailedReportOfCanceledTransactionsByRevenue.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await detailedReportOfCanceledTransactionsByRevenue.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await detailedReportOfCanceledTransactionsByRevenue.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await detailedReportOfCanceledTransactionsByRevenue.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await detailedReportOfCanceledTransactionsByRevenue.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    detailedReportOfCanceledTransactionsByRevenue.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await detailedReportOfCanceledTransactionsByRevenue.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await detailedReportOfCanceledTransactionsByRevenue.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await detailedReportOfCanceledTransactionsByRevenue.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    detailedReportOfCanceledTransactionsByRevenue.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    detailedReportOfCanceledTransactionsByRevenue.selectors.submitButton as string
  );
});
