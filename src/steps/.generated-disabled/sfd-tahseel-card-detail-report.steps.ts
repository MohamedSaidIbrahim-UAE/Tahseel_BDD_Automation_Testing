/**
 * Step Definitions for SFD Tahseel Card Detail Report Module
 *
 * This file contains all step definitions for the SFD Tahseel Card Detail Report feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module sfd-tahseel-card-detail-report.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { SfdTahseelCardDetailReport } from '../pages/generated/sfd-tahseel-card-detail-report.page';
 ...
 */
let page: Page;
let sfdTahseelCardDetailReport: SfdTahseelCardDetailReport;
...
function initializePageObject(): void {
  ...
  sfdTahseelCardDetailReport = new SfdTahseelCardDetailReport(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await sfdTahseelCardDetailReport.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await sfdTahseelCardDetailReport.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await sfdTahseelCardDetailReport.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await sfdTahseelCardDetailReport.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await sfdTahseelCardDetailReport.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await sfdTahseelCardDetailReport.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await sfdTahseelCardDetailReport.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    sfdTahseelCardDetailReport.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await sfdTahseelCardDetailReport.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await sfdTahseelCardDetailReport.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await sfdTahseelCardDetailReport.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    sfdTahseelCardDetailReport.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    sfdTahseelCardDetailReport.selectors.submitButton as string
  );
});
