/**
 * Step Definitions for Transaction Summary Tax Report - Revenue Module
 *
 * This file contains all step definitions for the Transaction Summary Tax Report - Revenue feature.
 * Steps are organized by type: Given, When, Then
 *
 * @category Step Definitions
 * @module transaction-summary-tax-report-revenue.steps
 */

import { Given, When, Then, DataTable } from '@cucumber/cucumber';
import { Page, expect } from '@playwright/test';
import { TransactionSummaryTaxReportRevenue } from '../pages/generated/transaction-summary-tax-report-revenue.page';
 ...
 */
let page: Page;
let transactionSummaryTaxReportRevenue: TransactionSummaryTaxReportRevenue;
...
function initializePageObject(): void {
  ...
  transactionSummaryTaxReportRevenue = new TransactionSummaryTaxReportRevenue(page);
}
...
Given('the user navigates to the "{string}" module', async (moduleName: string) => {
  ...
  await transactionSummaryTaxReportRevenue.navigateToModule();
});
...
Given('the module page is loaded', async () => {
  initializePageObject();
  await transactionSummaryTaxReportRevenue.verifyPageLoaded();
});
...
Given('the form is open', async () => {
  initializePageObject();
  await transactionSummaryTaxReportRevenue.verifyFormVisible();
});
...
Given('the user fills the form with the following data:', async (dataTable: DataTable) => {
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    await transactionSummaryTaxReportRevenue.fillField(fieldName, fieldValue);
  }
});
...
When('the user searches for "{string}"', async (searchTerm: string) => {
  initializePageObject();
  await transactionSummaryTaxReportRevenue.searchTable(searchTerm);
});
...
When('the user clicks the export button', async () => {
  initializePageObject();
  await transactionSummaryTaxReportRevenue.exportData();
});
...
When('the user submits the form', async () => {
  initializePageObject();
  await transactionSummaryTaxReportRevenue.submitForm();
});
...
Then('all main elements should be visible', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    transactionSummaryTaxReportRevenue.selectors.mainContainer as string
  );
});
...
Then('the table should contain at least one row', async () => {
  initializePageObject();
  await transactionSummaryTaxReportRevenue.verifyTableHasData();
});
...
Then('the success message should be displayed', async () => {
  initializePageObject();
  await transactionSummaryTaxReportRevenue.verifySuccessMessage();
});
...
Then('the form should close', async () => {
  initializePageObject();
  await transactionSummaryTaxReportRevenue.verifyFormClosed();
});
...
Then('the validation error should be displayed', async () => {
  ...
  await AssertionHelpers.assertElementVisible(
    page,
    transactionSummaryTaxReportRevenue.selectors.errorMessage as string
  );
});
...
Then('the submit button should be enabled', async () => {
  ...
  await AssertionHelpers.assertElementEnabled(
    page,
    transactionSummaryTaxReportRevenue.selectors.submitButton as string
  );
});
