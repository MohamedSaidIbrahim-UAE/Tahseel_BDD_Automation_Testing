Feature: Transaction Deposits Detail Report (receivable) - Entity
  As a user
  I want to interact with the Transaction Deposits Detail Report (receivable) module
  So that I can manage and track entity

  Background:
    Given the user is authenticated
    And the user navigates to the "Transaction Deposits Detail Report (receivable)" module
  # POSITIVE SCENARIOS

  Scenario: Load module page successfully
    When the user opens the module
    Then the module page should load
    And the page title should display "Transaction Deposits Detail Report (receivable)"
