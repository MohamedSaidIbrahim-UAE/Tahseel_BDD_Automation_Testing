Feature: Tax Report – Transaction Details Tax Report by Revenue Entity
  ...
  Background:
    ...
    And two entities "Entity-A" (standard) and "Entity-B" (tax-exempt) exist
  ...
  Scenario: Detailed tax view per revenue entity after transactions
    Given the following transactions are posted:
      ...
      | TXN-201     | Entity-A  | 500         | 5%       |
      | TXN-202     | Entity-A  | 1500        | 5%       |
      | TXN-203     | Entity-B  | 2000        | 0%       |
    When the user generates the "Transaction Details Tax Report - Revenue" and selects "Entity-A" as the filter
    Then only TXN-201 and TXN-202 are displayed
    And the total VAT for Entity-A is 100.00 AED (5% of 2000)

  @positive @e2e
  Scenario: Switching entity filter updates the report instantly
    When the user changes the revenue entity filter from "Entity-A" to "Entity-B"
  ...
  Scenario: Entity-restricted user can only view their own entity
    Given the user is logged in as a restricted "Entity-A Accountant" with scope limited to Entity-A
    When the user opens the "Transaction Details Tax Report - Revenue"
    Then the entity filter is locked to "Entity-A" or shows only Entity-A data by default
